...
def demo_sequential_vs_parallel():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        console.print(
            f"Created {len(demo_files)} demo files ({total_size / 1024 / 1024:.2f} MB total)"
        )
        ...
        try:
            ...
            console.print(
                "[yellow]Enhanced parallel coordinator not available[/yellow]"
            )
        ...
        try:
            ...
            sequential_files_per_sec = (
                sequential_result["processed_files"] / sequential_duration
            )
        ...
        try:
            ...
            parallel_files_per_sec = (
                parallel_result["processed_files"] / parallel_duration
            )
        ...
        if enhanced_available:
            ...
            try:
                ...
                enhanced_files_per_sec = (
                    enhanced_result["processed_files"] / enhanced_duration
                )
                ...
                if "performance" in enhanced_result:
                    ...
                    console.print(
                        f"  • Estimated Speedup: {perf.get('speedup_estimate', 1.0):.1f}x"
                    )

                if "memory_stats" in enhanced_result:
                    mem_stats = enhanced_result["memory_stats"]
                    console.print(
                        f"  • Peak Memory Usage: {mem_stats.get('peak_usage_mb', 0):.1f} MB"
                    )
                    console.print(
                        f"  • Memory Throttle Events: {mem_stats.get('throttle_events', 0)}"
                    )

                if "load_balancer_stats" in enhanced_result:
                    lb_stats = enhanced_result["load_balancer_stats"]
                    console.print(
                        f"  • Work Steal Events: {lb_stats.get('steal_events', 0)}"
                    )
                    console.print(
                        f"  • Load Rebalances: {lb_stats.get('rebalance_events', 0)}"
                    )
        ...
        if (
            hasattr(parallel_coordinator, "adaptive_config")
            and parallel_coordinator.adaptive_config
        ):
            ...
            config_table.add_row(
                "Section Parallelism", str(config.section_parallelism), ""
            )
...
def demo_adaptive_parallelism():
    ...
    from src.decompile.adaptive_parallelism import (
        get_adaptive_engine,
        optimize_for_files,
    )
...
def main():
    ...
    try:
        console.print(
            "[bold green]PowerRebuilder Parallel Processing Demo[/bold green]\n"
        )
        ...
        console.print(
            "  [cyan]sime-finch decompile --parallel input_dir output_dir[/cyan]"
        )
        console.print(
            "  [cyan]sime-finch decompile --parallel --max-workers 8 input_dir output_dir[/cyan]"
        )
        console.print(
            "  [cyan]sime-finch decompile --parallel --use-threads input_dir output_dir[/cyan]"
        )
