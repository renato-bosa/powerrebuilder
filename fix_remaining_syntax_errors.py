...
fixes_map = {
    "src/model/entities/method_call.py": [
        lambda c: c.replace("dynamic_class_expr: | None = None", "dynamic_class_expr: Any | None = None")
    ],
    ...
    "src/decompile/pcode/decoder.py": [
        ...
        lambda c: re.sub(r'(\s+)pcode_bytes = object_data\[pcode_offset.*?\]\n(\s+)instructions', 
    ...
    "src/decompile/pcode/opcodes/variants.py": [
        ...
        lambda c: re.sub(r'(\s+)elif low_nibble == 0x09:\n(\s+)values\.append', 
                        r'\1elif low_nibble == 0x09:\n\1    values.append', c)
    ],
    
    "src/extract/components/resources.py": [
        # Fix unmatched parenthesis
        lambda c: re.sub(r'found_resources\):\s*$', 'found_resources):', c, flags=re.MULTILINE)
    ],
    ...
    "src/decompile/core/formatter.py": [
        # Fix line 623
        lambda c: c.replace(") and i + 1 < len(decoded_obj.instructions):", 
                           " and i + 1 < len(decoded_obj.instructions):")
    ],
    
    "src/decompile/pcode/detector.py": [
        # Fix unmatched parenthesis in function parameter
        lambda c: re.sub(r'confidence: float = 0\.0\) -> None:', 
                        'confidence: float = 0.0) -> None:', c)
    ],
...
generic_fixes = [
    ...
    (r'(\n\s+)else:\s*\n', lambda m: m.group(0) if any(kw in m.string[max(0, m.start()-200):m.start()] 
                                                       for kw in ['if ', 'elif ']) else ''),
    
    # Fix elif without matching if  
    (r'(\n\s+)elif\s+', lambda m: m.group(0) if any(kw in m.string[max(0, m.start()-200):m.start()] 
                                                    for kw in ['if ', 'elif ']) else '\n' + m.group(1) + 'if '),
    
    # Fix except without try
    (r'(\n\s+)except\s+', lambda m: m.group(0) if 'try:' in m.string[max(0, m.start()-500):m.start()] 
                                                  else ''),
]
