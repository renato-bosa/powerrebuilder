...
class FinalSyntaxFixer:
    def __init__(self, backup_dir: str = "syntax_backup_final"):
        self.backup_dir = Path(backup_dir)
        self.backup_dir.mkdir(exist_ok=True)
        self.log_file = f"final_syntax_fixes_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        self.fixed_files = []
        self.failed_files = []
            ...
            if close_count > open_count:
                ...
                if total_close > total_open:
                    ...
                    for _ in range(extra):
                        if ')' in line:
                            line = line.replace(')', '', 1)
                            self.log(f"  Removed extra closing parenthesis at line {i+1}")
            
            # Fix missing closing parentheses in function definitions
            if line.strip().startswith('def ') and '(' in line and not ')' in line:
                line = line.rstrip() + '):\n'
                self.log(f"  Added missing closing parenthesis to function at line {i+1}")
        ...
        for i, line in enumerate(lines):
            ...
            if re.match(r'^\s*\w+\s*=\s*\w+\([^)]+$', line):
                ...
                if i + 1 < len(lines) and not ')' in lines[i + 1]:
                    line = line.rstrip() + ')\n'
                    self.log(f"  Added missing closing parenthesis to call at line {i+1}")
    ...
    def run(self):
        ...
        if self.failed_files:
            ...
            self.log("1. For 'unmatched )' errors: Look for incomplete function/class definitions")
            self.log("2. For 'invalid syntax' errors: Check for missing colons or commas")
            self.log("3. For 'unexpected indent' errors: Ensure consistent 4-space indentation")
...
def main():
    ...
    if failed > 0:
        print(f"\n{failed} files still need manual fixes. See log for details.")
        return 1
    else:
        print(f"\nAll {fixed} files fixed successfully!")
        return 0
