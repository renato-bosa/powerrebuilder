    ...
    while i < len(lines):
        ...
        if i + 2 < len(lines):
            ...
            if (next_line == "" and 
                ...
                len(third_line) > len(current_stripped) and
                third_line[len(current_stripped):].strip().startswith(':')):
            ...
            if (next_line.startswith(current_stripped) and 
                len(next_line) > len(current_stripped) and
                next_line[len(current_stripped):].strip().startswith(':')):
                # Skip the duplicate
                fixed_lines.append(line)
                i += 2
                continue
