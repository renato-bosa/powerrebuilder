...
def demo_basic_job_queue():
    ...
    for _ in range(5):
        ...
        if job:
            logger.info("Got job %s: %s (priority=%d)", 
                       job.id, job.task_name, job.priority)
...
def demo_pipeline_stages():
    ...
    def stage2_transform(extracted_items: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for item in extracted_items:
            logger.info("[PID %d] Transforming %s", process_id, item.get("file", "unknown"))
    ...
    def stage3_generate(transformed_items: List[Dict]) -> List[Dict[str, Any]]:
        ...
        for item in transformed_items:
            logger.info("[PID %d] Generating from %s", process_id, item.get("source", "unknown"))
    ...
    pipeline = DistributedPipeline(
        processor.job_queue,
        processor.worker_pool,
        processor.task_registry
    )
    ...
    try:
        ...
        transformed_data = [
            {"source": f"file_{i}.pbl", "transformed": True}
            for i in range(20)
        ]
...
def demo_fault_tolerance():
    ...
    def flaky_task(item_id: int) -> str:
        ...
        logger.info("[PID %d] Processing item %d (attempt %d)", 
                   process_id, item_id, attempt_counter[item_id])
