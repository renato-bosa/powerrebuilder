...
class OpcodeExtractor:
    ...
    def extract_from_pbdviewer(self):
        """Extract opcodes from C# pbdviewer implementation."""
        print("📖 Extracting from pbdviewer (C#)...")
        ...
        for parser_file in Path("reference/pbdviewer/Uitils/PCode").glob("PCodeParser*.cs"):
            with open(parser_file, 'r') as f:
                content = f.read()
                
            # Extract byte array definitions
            array_match = re.search(r'byte\[\]\s*.*?=\s*new\s*byte\[\d+\]\s*\{([^}]+)\}', content, re.DOTALL)
            if array_match:
                lengths_str = array_match.group(1)
                lengths = [int(x.strip()) for x in lengths_str.split(',') if x.strip().isdigit()]
        ...
        self.implementations['pbdviewer'] = len([o for o in self.opcodes.values() if 'pbdviewer_handler' in o])
        
    def extract_from_powerbuilder_decompile(self):
        """Extract opcodes from Python powerbuilder-decompile."""
        print("📖 Extracting from powerbuilder-decompile (Python)...")
            ...
            if g_codes_match:
                ...
                for entry in entries:
                    ...
                    if index_match:
                        ...
                        if name_match:
                            self.opcodes[opcode]['pb_decompile_name'] = name_match.group(1)
                        if func_match:
                            self.opcodes[opcode]['pb_decompile_func'] = func_match.group(1)
                        if args_match:
                            self.opcodes[opcode]['pb_decompile_args'] = int(args_match.group(1))
            
            # Extract function implementations for stack effects
            func_pattern = r'def\s+(pb_\w+)\(stack,\s*pcode,\s*routine\):(.*?)(?=\ndef|\Z)'
        ...
        self.implementations['powerbuilder-decompile'] = len([o for o in self.opcodes.values() if 'pb_decompile_name' in o])
    
    def extract_from_sime_finch(self):
        """Extract our verified opcodes."""
        print("📖 Extracting from sime-finch verified opcodes...")
        ...
        self.implementations['sime-finch'] = len([o for o in self.opcodes.values() if 'verified_name' in o])
    
    def analyze_patterns(self):
        """Analyze patterns in opcode definitions."""
        print("\n📊 Analyzing opcode patterns...")
        
        # Group by operation type
        operation_types = defaultdict(list)
        for opcode, info in self.opcodes.items():
            name = info.get('pb_decompile_name') or info.get('verified_name') or 'UNKNOWN'
            if '_' in name:
        ...
        for opcode, info in self.opcodes.items():
            ...
            if name:
                ...
                for dtype in ['INT', 'UINT', 'LONG', 'ULONG', 'DOUBLE', 'DEC', 'FLOAT', 
                             'STRING', 'BOOL', 'BINARY', 'DATE', 'TIME', 'CHAR', 'ANY']:
    ...
    def generate_comprehensive_reference(self):
        ...
        print("\n📝 Generating comprehensive opcode reference...")
        ...
        for opcode in sorted(self.opcodes.keys()):
            ...
            # Determine best name
            name = (info.get('verified_name') or 
                   info.get('pb_decompile_name', '').replace('SM_', '') or
        ...
        reference['patterns'] = {
            'operation_types': {k: [f'0x{o:02X}' for o in v] for k, v in operation_types.items()},
    ...
    def generate_markdown_docs(self, reference, output_path):
        ...
        with open(output_path, 'w') as f:
            ...
            f.write(f"- Operation types: {reference['statistics']['operation_types']}\n")
            f.write(f"- Type variants: {reference['statistics']['type_variants']}\n")
            
            f.write("\n## Opcode Listing\n\n")
            f.write("| Opcode | Name | Length | Category | Stack Effect | Confidence |\n")
            f.write("|--------|------|--------|----------|--------------|------------|\n")
            ...
            for opcode_hex, info in reference['opcodes'].items():
                ...
                f.write(f"| {opcode_hex} | {name} | {length} | {category} | {stack} | {confidence} |\n")
...
def main():
    ...
    # Print summary
    print("\n📊 Extraction Summary:")
    print(f"  Total unique opcodes: {len(extractor.opcodes)}")
    for impl, count in extractor.implementations.items():
