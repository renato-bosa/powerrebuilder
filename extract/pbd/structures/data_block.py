...
from ..utils.binary_utils import binary_to_int, retrieve_bytes_from_file
...
DAT_DATA_LEN_FIELD_LEN = 4  # Data length is a 4-byte integer

# The actual data starts after the DAT header (sig, next_offset, data_len)
DAT_HEADER_SIZE_ASCII = DAT_SIGNATURE_LEN_ASCII + DAT_NEXT_BLOCK_OFFSET_FIELD_LEN + DAT_DATA_LEN_FIELD_LEN
DAT_HEADER_SIZE_UNICODE = DAT_SIGNATURE_LEN_UNICODE + DAT_NEXT_BLOCK_OFFSET_FIELD_LEN + DAT_DATA_LEN_FIELD_LEN
...
class DataClass:
    ...
    is_unicode_data_block_header: bool  # True if this DAT block's header indicates Unicode (D\0A\0T\0)
...
def extract_data_from_entry(
    ...
    while current_block_offset != 0:
        ...
        potential_header_bytes = retrieve_bytes_from_file(
            file_handle, current_block_offset, max_dat_header_size, block_size_override=block_size,
        )

        if not potential_header_bytes or len(potential_header_bytes) < min(DAT_HEADER_SIZE_ASCII, DAT_HEADER_SIZE_UNICODE):
            logger.error(
        ...
        if potential_header_bytes.startswith(dat_sig_unicode):
            ...
            next_block_offset_in_header = binary_to_int(potential_header_bytes[DAT_NEXT_BLOCK_OFFSET_FIELD_OFFSET_UNICODE : DAT_NEXT_BLOCK_OFFSET_FIELD_OFFSET_UNICODE + DAT_NEXT_BLOCK_OFFSET_FIELD_LEN])
            data_len_in_header = binary_to_int(potential_header_bytes[DAT_DATA_LEN_FIELD_OFFSET_UNICODE : DAT_DATA_LEN_FIELD_OFFSET_UNICODE + DAT_DATA_LEN_FIELD_LEN])
            ...
            next_block_offset_in_header = binary_to_int(potential_header_bytes[DAT_NEXT_BLOCK_OFFSET_FIELD_OFFSET_ASCII : DAT_NEXT_BLOCK_OFFSET_FIELD_OFFSET_ASCII + DAT_NEXT_BLOCK_OFFSET_FIELD_LEN])
            data_len_in_header = binary_to_int(potential_header_bytes[DAT_DATA_LEN_FIELD_OFFSET_ASCII : DAT_DATA_LEN_FIELD_OFFSET_ASCII + DAT_DATA_LEN_FIELD_LEN])
        else:
            logger.error(
        ...
        if data_offset_in_file + bytes_to_read_for_data > file_size:
            logger.warning(
                f"DAT block for '{entry_def.objectname}' at offset {current_block_offset}: Declared data length {bytes_to_read_for_data} "
                f"extends beyond file size {file_size} (ends at {data_offset_in_file + bytes_to_read_for_data}). Reading up to EOF. Marking as partial.",
            )
            bytes_to_read_for_data = file_size - data_offset_in_file  # Adjust to read only available bytes
            bytes_to_read_for_data = max(bytes_to_read_for_data, 0)
            is_partial = True  # Data will be truncated

        actual_data_bytes = b''
        if bytes_to_read_for_data > 0:
            actual_data_bytes = retrieve_bytes_from_file(
                file_handle, data_offset_in_file, bytes_to_read_for_data, block_size_override=block_size,
            )
            if not actual_data_bytes or len(actual_data_bytes) < bytes_to_read_for_data:
                ...
                if not actual_data_bytes: actual_data_bytes = b''  # Ensure it's bytes

        data_block = DataClass(
            address=current_block_offset,
            data=actual_data_bytes,
            next_block_offset=next_block_offset_in_header,
            data_length_in_block=len(actual_data_bytes),  # Store actual bytes read for this block
        ...
        if is_partial and current_block_offset != 0:
            logger.info(f"DAT chain for '{entry_def.objectname}' is partial, but next_block_offset is {current_block_offset}. Stopping chain to prevent further errors.")
...
def get_text_from_data(all_data_blocks: list[DataClass], is_unicode_file: bool) -> str:
    ...
    for x in all_data_blocks:
        try:
            ...
            logger.warning(f"Unicode decode error in DAT block at 0x{x.address:X} with encoding '{encoding}'. Error: {ude}. Data (hex): {x.data[:32].hex()}...")
            text += f"<DECODE_ERROR: {encoding}>"  # Placeholder
        except Exception as e:
            logger.error(f"Unexpected error decoding DAT block at 0x{x.address:X} with encoding '{encoding}'. Error: {e}. Data (hex): {x.data[:32].hex()}...", exc_info=True)
