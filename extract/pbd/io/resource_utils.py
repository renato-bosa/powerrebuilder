    ...
    if offset + 6 <= len(data):  # Initial header: 2 reserved, 2 type, 2 count
        try:
            ...
            for _ in range(num_images):
                if current_dir_entry_offset + 16 > len(data):  # ICONDIRENTRY is 16 bytes
                ...
                img_size_bytes = struct.unpack_from('<I', data, current_dir_entry_offset + 8)[0]
                img_offset_bytes = struct.unpack_from('<I', data, current_dir_entry_offset + 12)[0]

                max_end_offset = max(max_end_offset, offset + img_offset_bytes + img_size_bytes)
    ...
    while idx + 8 <= len(data):  # Need at least 8 bytes for a chunk (length + type)
        try:
            ...
            if chunk_type == b'IEND':
                return (idx + 8 + chunk_len + 4) - offset  # Total size: offset to IEND + IEND_header (8) + chunk_data_len (0) + CRC (4)

            idx += (4 + 4 + chunk_len + 4)  # Move to next chunk: len_field + type_field + data + crc
        except struct.error:
...
SIGNATURE_MAP = {
    BMP_SIGNATURE: {"ext": ".bmp", "name": "BMP", "get_size": get_bmp_size, "min_size": 54},  # Min BMP header size
    ICO_SIGNATURE: {"ext": ".ico", "name": "ICO", "get_size": get_ico_size, "min_size": 22},  # 6 (header) + 16 (direntry)
    b"\\x89PNG\\r\\n\\x1a\\n": {"ext": ".png", "name": "PNG", "get_size": get_png_size, "min_size": 24},  # 8 (sig) + IHDR chunk (12+header) + ...
    b"\\xFF\\xD8\\xFF": {"ext": ".jpg", "name": "JPG", "get_size": get_jpg_size, "min_size": 20},  # SOI, APP0, ... EOI
    b"GIF87a": {"ext": ".gif", "name": "GIF", "get_size": get_gif_size, "min_size": 13},  # Header + LSD
    b"GIF89a": {"ext": ".gif", "name": "GIF", "get_size": get_gif_size, "min_size": 13},  # Header + LSD
}
...
def extract_embedded_images(
    ...
    potential_images: list[tuple[int, dict, int]] = []  # (start_offset, image_info, signature_length)
    ...
    for sig_bytes, img_info in SIGNATURE_MAP.items():
        ...
        while start_search_idx < len(data_bytes):
            ...
            start_search_idx = found_at + 1  # Look for next occurrence of *this* signature
    ...
    while current_data_offset < len(data_bytes):
        ...
        logger.debug(f"Potential {image_name} signature found at offset {start_of_image}.")
        ...
        if get_size_func:
            ...
            if determined_size and (start_of_image + determined_size) <= len(data_bytes) and determined_size >= min_img_size:
                image_data = data_bytes[start_of_image : start_of_image + determined_size]
                logger.debug(f"{image_name} at {start_of_image}: Determined size {determined_size} bytes.")
            elif determined_size:
                 logger.warning(f"{image_name} at {start_of_image}: Declared size {determined_size} is invalid or too small. Min expected {min_img_size}. Will skip or use fallback if any.")
        ...
        if not image_data:
            ...
            for next_sig_start, _, _ in potential_images:
                if next_sig_start > start_of_image:  # Must be after current image's start
                    end_of_image_heuristic = min(end_of_image_heuristic, next_sig_start)
                    break  # Take the very next one as a boundary

            if end_of_image_heuristic > start_of_image and (end_of_image_heuristic - start_of_image) >= min_img_size:
                image_data = data_bytes[start_of_image : end_of_image_heuristic]
                logger.debug(f"{image_name} at {start_of_image}: Using heuristic end at {end_of_image_heuristic}. Size: {len(image_data)}")
            elif not determined_size:  # Only log if we didn't already warn about bad determined_size
                 logger.warning(f"{image_name} at {start_of_image}: Heuristic extraction resulted in too small size or no data. Min expected {min_img_size}. Skipping.")
                 image_data = None
        ...
        if image_data:
            try:
                ...
                logger.info(f"Extracted embedded {image_name} to {image_path} (bytes: {len(image_data)})")
                image_file_idx += 1
                processed_offsets.add(start_of_image)  # Mark this offset as processed
                current_data_offset = start_of_image + len(image_data)  # Continue scan from end of this image
            except OSError as e:
                logger.error(f"Failed to write extracted image {image_filename}: {e}")
                processed_offsets.add(start_of_image)
                current_data_offset = start_of_image + signature_len  # Try to skip problematic signature
            except Exception as e:  # General errors
                logger.error(f"Unexpected error extracting {image_name} from offset {start_of_image}: {e}")
                processed_offsets.add(start_of_image)
            ...
            logger.debug(f"Could not extract valid data for {image_name} at offset {start_of_image}. Advancing past signature.")
            processed_offsets.add(start_of_image)
            current_data_offset = start_of_image + signature_len
