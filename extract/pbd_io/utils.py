...
def bin2int(data: bytes, signed: bool = False) -> int:
    ...
    if not data:  # Handle empty byte string
        # logger.warning("bin2int received empty byte string. Returning 0.")
        return 0
    try:
        if signed:
            if length == 4:
            ...
            logger.warning(f"bin2int: Unsupported byte length {length} for signed conversion. Returning 0.")
            return 0
        # Unsigned
        if length == 8:
        ...
        # logger.warning(f"bin2int: Unsupported byte length {length} for unsigned. Returning 0.")
        return int.from_bytes(data, byteorder='little', signed=False)  # Fallback for other lengths
    except struct.error:
        # logger.warning(f"bin2int: struct.error for data {data.hex()} (len {len(data)}): {e}. Returning 0.")
        return 0  # Or raise a custom error


def bin2time(data: bytes) -> datetime.datetime:
    """Convert 4-byte little-endian integer timestamp to datetime object.
    Returns epoch (1970-01-01) on error.
    """
    if len(data) != 4:
        # logger.warning(f"bin2time: Expected 4 bytes, got {len(data)}. Data (hex): {data.hex()}. Returning epoch.")
        return datetime.datetime.fromtimestamp(0)
    try:
        timestamp = struct.unpack("<I", data)[0]
        return datetime.datetime.fromtimestamp(timestamp)
    except (struct.error, OSError, OverflowError):
        # logger.warning(f"bin2time: Error converting bytes to datetime: {e}. Data (hex): {data.hex()}. Returning epoch.")
        return datetime.datetime.fromtimestamp(0)


def do_nothing(arr: bytes) -> bytes:
    return arr


def ignore_bytes(b: bytes) -> None:
    return None
...
def retrieve_bytes_from_file(
    ...
    Args:
        ...
        block_size_override: If provided, this block size is used for padding calculations.
                           Otherwise, the default BLOCK_SIZE is used.
    ...
    """
    data = b""
    used_block_size = block_size_override if block_size_override is not None else BLOCK_SIZE
...
# List/map manipulation functions - review if these are still needed or can be refactored/inlined
def lst_of_offset(lst: list[Any]) -> list[Any]:
    return [x.offset for x in lst if hasattr(x, 'offset')]


def lst_data_2_map(lst_of_data: list[Any]) -> dict[Any, Any]:
    return {x.address: x for x in lst_of_data if hasattr(x, 'address')}


def lst_data_2_inverted_map(lst_of_data: list[Any]) -> dict[Any, Any]:
    return {x.offset: x.address for x in lst_of_data if hasattr(x, 'offset') and hasattr(x, 'address')}


def lst_of_addresses_offset(lst_of_data: list[Any]) -> list[Any]:
    return [(x.address, x.offset) for x in lst_of_data if hasattr(x, 'address') and hasattr(x, 'offset')]


def lst_of_addresses(lst_of_data: list[Any]) -> list[Any]:
    return [x.address for x in lst_of_data if hasattr(x, 'address')]


def lst_address_from_map(map_of_data: dict[Any, Any]) -> list[Any]:
    return list(map_of_data.keys())
