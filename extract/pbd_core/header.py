...
from extract.pbd_core.exceptions import HeaderError
from extract.pbd_io.utils import bin2int, bin2time, decode, extract_bytes_2_lst
...
def extract_pbl_header(
    ...
    header_and_fre_check_len = max(sum(HEADER_BLOCK_SIZES_UNICODE), sum(HEADER_BLOCK_SIZES_NON_UNICODE)) + (block_size * 2)

    if hasattr(file_input, 'seek') and hasattr(file_input, 'read') and hasattr(file_input, 'tell'):  # It's a BinaryIO handle
        handle = file_input
        ...
        try:
            ...
            raise HeaderError(f"Error reading from file handle {log_path}: {str(e)}")
    elif isinstance(file_input, bytes):
        file_bytes_for_header = file_input[:header_and_fre_check_len]  # Take only necessary part
        ...
        raise HeaderError(f"Unsupported file_input type: {type(file_input)}. Expected BinaryIO or bytes.")

    if not file_bytes_for_header or len(file_bytes_for_header) == 0:
        raise HeaderError(f"No header bytes to process for {file_path_for_error_log}.")
    ...
    if len(file_bytes_for_header) < min_buf_len_for_detection:  # Check against bytes read for header
         logger.warning(f"Header data for {file_path_for_error_log} is short ({len(file_bytes_for_header)} bytes, need ~{min_buf_len_for_detection} for full check). Some detections might be limited.")
    ...
    if file_bytes_for_header.startswith(b"HDR\\0"):  # Non-unicode
        ...
        detected_signature_bytes = file_bytes_for_header[:8]  # Unicode signature is 8 bytes 'H\\0D\\0R\\0*\\0'
        detected_is_unicode = True
    else:
        peek_bytes = file_bytes_for_header[:8].hex()
        raise HeaderError(f"Invalid PBL header signature for {file_path_for_error_log}. Expected HDR\\0 or HDR*. Got bytes: {peek_bytes}")
    ...
    if detected_is_unicode:  # FRE* check only relevant for HDR* files
        ...
        if len(file_bytes_for_header) >= fre_check_offset + 4:  # Need at least 4 bytes for 'FRE*'
            potential_fre_block_sig_ascii_bytes = file_bytes_for_header[fre_check_offset : fre_check_offset + 4]
            if potential_fre_block_sig_ascii_bytes == b'FRE*':
                logger.info(f"HDR* file ({file_path_for_error_log}): ASCII FRE* signature found at offset {fre_check_offset}. Adjusting first NOD offset by +{block_size} bytes.")
                final_first_nod_offset = initial_nod_offset + block_size
            else:
                logger.info(f"HDR* file ({file_path_for_error_log}): No ASCII FRE* signature found at offset {fre_check_offset}. Bytes found (first 4): {potential_fre_block_sig_ascii_bytes.hex() if potential_fre_block_sig_ascii_bytes else 'None'}")
        else:
            logger.warning(f"HDR* file ({file_path_for_error_log}): Buffer too short (len {len(file_bytes_for_header)}) to check for FRE* signature at offset {fre_check_offset}.")

    # Parse the header fields
    required_header_len = sum(header_block_sizes_to_use)
    if len(file_bytes_for_header) < required_header_len:
        raise HeaderError(f"File buffer for {file_path_for_error_log} too short for PBL header. Need {required_header_len}, have {len(file_bytes_for_header)} bytes.")
    ...
    parsed_header_fields = extract_bytes_2_lst(file_bytes_for_header[:required_header_len], header_block_sizes_to_use, actual_functors)

    if len(parsed_header_fields) != len(HEADER_CLASS_ATTR_NAMES):  # Compares against the expected number of base fields
        raise HeaderError(f"Header parsing for {file_path_for_error_log} failed, expected {len(HEADER_CLASS_ATTR_NAMES)} fields, got {len(parsed_header_fields)}: {parsed_header_fields}")
    ...
    final_header_values = parsed_header_fields + [effective_is_unicode, final_first_nod_offset, detected_signature_bytes, input_file_size]
    ...
    try:
        ...
        logger.error(f"TypeError during HeaderClass instantiation for {file_path_for_error_log}. Values count: {len(final_header_values)}, HeaderClass expects fields: {expected_fields_info}. Error: {e}")
        logger.error(f"Values provided: {final_header_values}")
        raise HeaderError(f"Failed to create HeaderClass object for {file_path_for_error_log}. Values: {final_header_values}. Error: {e}")
