...
def extract_entry_def(arr: bytes) -> PbEntryDefinition | None:
    ...
    try:
        ...
        try:
            ...
            offset_int = bin2int(arr[8:12])
            obj_size_int = bin2int(arr[12:16])
            mod_time_int_bytes = arr[16:20]
            mod_time_dt = bin2time(mod_time_int_bytes)
            comment_len_int = bin2int(arr[20:22])
            obj_name_actual_len = bin2int(arr[22:24])
...
def extract_entry_def_unicode(arr: bytes) -> PbEntryDefinition | None:
    ...
    try:
        ...
        try:
            ...
            offset_int = bin2int(arr[16:24])
            obj_size_int = bin2int(arr[24:32])
            mod_time_int_bytes = arr[32:40]
            mod_time_dt = bin2time(mod_time_int_bytes)
            comment_len_int = bin2int(arr[40:44])
            obj_name_actual_len = bin2int(arr[44:48])
...
def extract_entry_def_mixed_mode(arr: bytes) -> PbEntryDefinition | None:
    ...
    try:
        ...
        try:
            ...
            offset_int = bin2int(offset_int_bytes)
            ...
            obj_size_int = bin2int(obj_size_int_bytes)
            ...
            mod_time_dt = bin2time(mod_time_int_bytes)  # Convert here
            ...
            comment_len_int = bin2int(comment_len_int_bytes)
...
def extract_object_name_len_from_entry(entry: bytes) -> int:
    ...
    functors: list[Callable[[bytes], Any]] = [decode, decode, bin2int, bin2int, bin2time, bin2int, bin2int]
...
def read_and_parse_entry_def(
    ...
    try:
        ...
        obj_name_len_val = bin2int(obj_name_len_bytes)
