...
class GenerateCoordinator:
    ...
    async def execute(self) -> StageResult:
        ...
        if not self.use_case:
            ...
            if hasattr(self, 'generator') and self.generator:
                ...
                self.logger.warning("No generator implementation available, using basic template generator")
    ...
    async def _direct_generation(self) -> StageResult:
        ...
        try:
            ...
            self.logger.info(f"Generating {self.target} code using {self.generator.__class__.__name__}")
    ...
    def _json_to_semantic_object(self, obj_data: dict) -> "SemanticObject":
        ...
        from ...domain.models import SemanticObject, ObjectType, Property, Method, Parameter, Event
    ...
    async def _basic_code_generation(self) -> StageResult:
        ...
        try:
            ...
            if self.target == "flutter":
                ...
                self.logger.warning(f"Unknown target: {self.target}, generating generic stub")
...
sqlmodel>=0.0.8
'''
        await self.filesystem.write_text(self.output_path / "requirements.txt", requirements)
  ...
  "browserslist": {{
    ...
    "development": [
      ...
      "last 1 safari version"
        ...
        await self.filesystem.write_text(self.output_path / "package.json", package_json)
...
'''
        await self.filesystem.write_text(self.output_path / "vite.config.ts", vite_config)
...
'''
        await self.filesystem.write_text(self.output_path / "src" / "main.tsx", main_tsx)
...
'''
        await self.filesystem.write_text(
            self.output_path / "src" / "main.rs", main_rs
        )
