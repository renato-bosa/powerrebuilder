...
from extract import AdvancedPBLExtractor, ExtractCoordinator
...
from _core import ObjectType, TargetLanguage
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('pbd_src_new.log')
    ]
)
...
class PBDPipeline:
    ...
    def process_file(self, pbd_file: Path) -> Dict:
        ...
        self.results['stages']['extract']['success' if extract_success else 'failed'] += 1

        if not extract_success:
            logger.warning(f"Extraction failed for {pbd_file.name}, skipping remaining stages")
            return file_result
        ...
        self.results['stages']['decompile']['success' if decompile_success else 'failed'] += 1
        ...
        if parse_success:
            ...
            self.results['stages']['model']['success' if model_success else 'failed'] += 1
            ...
            if model_success:
                ...
                self.results['stages']['generate']['success' if generate_success else 'failed'] += 1
    ...
    def process_all(self, pbd_dir: Path, test_mode: bool = False) -> Dict:
        ...
        self.results['summary'] = {
            ...
            'successful': sum(1 for f in self.results['files_processed'] if f.get('success')),
            'total_size_mb': sum(f['size'] for f in self.results['files_processed']) / 1024 / 1024
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process PBD files with src_new pipeline')
    parser.add_argument(
        '--input-dir',
    ...
    parser.add_argument(
        '--output-dir',
        default='output/src_new_pipeline',
