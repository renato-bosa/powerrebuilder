...
from ...domain.models import (
    ApplicationModel, SemanticObject, GeneratedProject, GeneratedFile,
    TargetLanguage, ObjectType
)
...
class DioxusGenerator(GeneratorPort):
    ...
    def generate_file(self, obj: SemanticObject, target: TargetLanguage) -> GeneratedFile:
    ...
    def generate(self, model: ApplicationModel, config: Dict[str, Any]) -> GeneratedProject:
        ...
        objects = model.objects.values() if isinstance(model.objects, dict) else model.objects
        for obj in objects:
            if obj.type == ObjectType.WINDOW:
    ...
    def _generate_cargo_toml(self, project: GeneratedProject, model: ApplicationModel) -> None:
    ...
    def _generate_main_rs(self, project: GeneratedProject, model: ApplicationModel) -> None:
    ...
    def _generate_build_rs(self, project: GeneratedProject, model: ApplicationModel) -> None:
        """Generate build.rs file."""
        project.files.append(GeneratedFile(
            path="build.rs",
            content=DIOXUS_BUILD_RS_TEMPLATE,
            language="rust"
        ))
    
    def _generate_styles(self, project: GeneratedProject, model: ApplicationModel) -> None:
        """Generate CSS styles."""
        project.files.append(GeneratedFile(
    ...
    def _generate_window_component(self, project: GeneratedProject, window: SemanticObject, model: ApplicationModel) -> None:
    ...
    def _generate_datawindow_component(self, project: GeneratedProject, datawindow: SemanticObject, model: ApplicationModel) -> None:
    ...
    def _generate_database_module(self, project: GeneratedProject, model: ApplicationModel) -> None:
...
struct {self._to_pascal_case(obj.name)} {{"""
                ...
                for prop in obj.properties:
                    rust_type = self._pb_to_rust_type(prop.type if hasattr(prop, 'type') else "string")
                    field_name = self._to_snake_case(prop.name if hasattr(prop, 'name') else "field")
    ...
    def _generate_feature_list(self, model: ApplicationModel) -> str:
        ...
        datawindow_count = sum(1 for obj in objects if obj.type == ObjectType.DATAWINDOW)
...
async fn delete_{table_name}(db: &SqlitePool, id: i64) -> Result<(), String> {{
    ...
    def _generate_table_headers(self, datawindow: SemanticObject) -> str:
        ...
        for prop in datawindow.properties:
            if hasattr(prop, 'type') and prop.type == "column":
                col_name = prop.name.replace("_", " ").title() if hasattr(prop, 'name') else "Column"
                headers.append(f"""
                        th {{
                            onclick: move |_| {{
    ...
    def _has_database_operations(self, model: ApplicationModel) -> bool:
        ...
        for obj in objects:
            ...
            for method in obj.methods:
                if any(keyword in getattr(method, 'body', '').upper() 
    ...
    def _is_data_object(self, obj: SemanticObject) -> bool:
        ...
        return obj.type == ObjectType.STRUCTURE or (
            obj.type == ObjectType.USER_OBJECT and 
            any(hasattr(p, 'type') and p.type == "datatype" for p in obj.properties)
        )
