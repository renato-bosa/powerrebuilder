...
from .exceptions import GrammarNotFoundError, ParseError
...
class GrammarManager:
    ...
    def __init__(self, grammar_dir: Optional[Path] = None):
        ...
        if not self.grammar_dir.exists():
            raise GrammarNotFoundError(f"Grammar directory not found: {self.grammar_dir}")
    ...
    def load_grammar(self, name: str, start: Optional[str] = None, **kwargs) -> Lark:
        ...
        try:
            ...
            logger.error(f"Grammar error in {name}: {e}")
            raise
        except Exception as e:
            logger.error(f"Failed to create parser for {name}: {e}")
            raise GrammarError(f"Failed to create parser for {name}: {e}")
    ...
    def _load_grammar_content(self, name: str) -> str:
        ...
        if not grammar_file.exists():
            raise GrammarNotFoundError(f"Grammar file not found: {grammar_file}")
        
        # Read grammar file
        try:
            content = grammar_file.read_text(encoding="utf-8")
        except Exception as e:
            raise GrammarError(f"Failed to read grammar file {grammar_file}: {e}")
    ...
    def get_parser(self, file_type: Union[FileType, str]) -> Lark:
        ...
        if not grammar_name:
            raise ValueError(f"No grammar mapping for file type: {file_type}")
    ...
    def get_grammar_info(self) -> dict[str, dict]:
        ...
        for name in self._grammars:
            info[name] = {
                ...
                "cached_parsers": sum(1 for k in self._cache if k.startswith(f"{name}:")),
...
_default_manager: Optional[GrammarManager] = None
