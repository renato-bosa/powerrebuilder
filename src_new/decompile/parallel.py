...
class ParallelDecoder:
    ...
    async def _async_decode_batch(self, files: List[Path]) -> Dict[Path, DecompileResult]:
        ...
        return {
            file: result if not isinstance(result, Exception) else DecompileResult(
                success=False,
                source="",
                instructions=[],
                errors=[str(result)],
...
class StreamingDecoder:
    ...
    async def decode_stream_async(self, file_path: Path) -> AsyncIterator[StreamingResult]:
        """Async stream decode.
