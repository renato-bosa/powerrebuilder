...
class PCodeDecoder:
    ...
    def decode_file(self, file_path: Path) -> List[str]:
        """Decode a PowerBuilder binary file to text format.
        
        Args:
            file_path: Path to the binary file
            
        Returns:
            List of text-formatted instructions for decompile_structured.py
        """
        ...
        if pcode_start < 0:
            logger.error(f"Could not find P-code start in {file_path}")
            return []
        ...
        return self._format_instructions_as_text(instructions)
    ...
    def _identify_jump_targets(self, pcode: bytes, base_offset: int):
    ...
    def _decode_next_instruction(self, pcode: bytes, base_offset: int) -> Optional[PCodeInstruction]:
        ...
        if opcode_info:
            ...
            elif 'variants' in opcode_info and len(remaining) > 1:
    ...
    def _decode_unknown_opcode(self, address: int, data: bytes, full_pcode: bytes) -> PCodeInstruction:
        ...
        fallback = attempt_symbolic_fallback(
            opcode_value=opcode_value,
...
def decode_and_save(input_file: Path, output_file: Path):
    """Decode a binary P-code file and save as text."""
    decoder = PCodeDecoder()
    
    logger.info(f"Decoding {input_file}")
    text_lines = decoder.decode_file(input_file)
    
    if text_lines:
        output_file.parent.mkdir(parents=True, exist_ok=True)
        with open(output_file, 'w') as f:
            f.write('\n'.join(text_lines))
        logger.info(f"Saved decoded P-code to {output_file}")
        return True
    else:
        logger.error(f"Failed to decode {input_file}")
        return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2:
        ...
        decode_and_save(input_path, output_path)
    elif len(sys.argv) > 1:
        input_path = Path(sys.argv[1])
        output_path = Path(sys.argv[1]).with_suffix('.pcode')
        decode_and_save(input_path, output_path)
    else:
        # Test with a sample file
        test_file = Path("output/test_bytes_fix/dcm_accounting.pbd/dcm_accounting.pbd/of_get_linked_acc.fun")
        if test_file.exists():
            output_file = test_file.with_suffix('.pcode')
            decode_and_save(test_file, output_file) 