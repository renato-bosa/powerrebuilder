...
class ExpressionLifter:
    ...
    def lift_instruction_sequence(self, instructions: list[PCodeInstruction]) -> list[Expression | str]:
    ...
    def lift_instruction(self, inst: PCodeInstruction) -> Expression | str | None:
        ...
        # Binary operations
        if opcode in self._get_binary_ops():
            self._handle_binary_op(opcode)
            return None

        # Unary operations
        if opcode in self._get_unary_ops():
        ...
        # Function calls
        if self._is_call_opcode(opcode):
            return self._handle_call(inst)

        # Control flow
        if opcode in ["RETURN", "HALT", "EXIT"]:
        ...
        if opcode in op_map:
            ...
            if operator in ["++", "--"]:
                ...
                if operand.type == ExpressionType.VARIABLE:
                    ...
                    self.stack.append(expr)
                    return
                if operand.type == ExpressionType.CALL:
    ...
    def _infer_binary_type(self, left_type: str | None, right_type: str | None,
                          operator: str) -> str | None:
    ...
    def _handle_assign(self, inst: PCodeInstruction) -> str | None:
