...
class PCodeToSource:
    ...
    def decompile_datawindow(self, instructions: List[PCodeInstruction], name: str) -> str:
        ...
        source_lines.append(f"release 10;")
        source_lines.append(f'datawindow(units=0 timer_interval=0 processing=0 HTMLDW=no)')
        source_lines.append("")
    ...
    def _find_expression_end(self, instructions: List[PCodeInstruction], start: int) -> int:
        ...
        while i < len(instructions):
            ...
            elif inst.opcode_name in ['JUMP', 'JUMP_IF_FALSE', 'JUMP_IF_TRUE', 'CALL_FUNCTION', 'CALL_METHOD']:
                return i - 1
            i += 1
        return start
    ...
    def _process_single_instruction(self, inst: PCodeInstruction) -> Optional[str]:
...
def main():
    ...
    for file_path in test_files:
        if file_path.exists():
            print(f"\n{'='*60}")
            print(f"Decompiling: {file_path.name}")
            print(f"{'='*60}")
