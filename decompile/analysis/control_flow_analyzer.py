...
class ControlFlowAnalyzer:
    ...
    JUMP_OPCODES = {
        ...
        'JUMPIF', 'JUMPIFNOT', 'BR', 'BRA',
    }
    
    # Opcodes that unconditionally terminate a block
    UNCONDITIONAL_TERMINATORS = {
        'JUMP', 'JMP', 'BR', 'BRA', 'HALT', 'THROW', 
        'RETHROW', 'EXIT', 'RETURN', 'RET',
    }
    ...
    def _is_terminator(self, inst: PCodeInstruction) -> bool:
        ...
        return (inst.opcode_name in self.UNCONDITIONAL_TERMINATORS or 
    ...
    def _try_match_if(self, blocks: list[ControlBlock], start_idx: int,
        ...
        if last_inst.opcode_name in ['JUMPFALSE', 'BRFALSE']:
            ...
            if target_idx > start_idx:
                for idx in range(target_idx, len(blocks)):
                    ...
                    if blocks[idx].instructions and blocks[idx].instructions[-1].opcode_name == 'JUMP':
                        break
        ...
        if (if_block.then_block and 
            ...
            if_block.then_block.instructions[-1].opcode_name in ['JUMP', 'JMP']):
            
            else_jump_target = self._get_jump_target_address(if_block.then_block.instructions[-1])
            if else_jump_target:
            ...
            if last_inst.opcode_name in ['JUMP', 'JMP', 'JUMPTRUE', 'BRTRUE']:
                ...
                if target is not None and target <= blocks[start_idx].start_addr:
                    ...
                    while_block = ControlBlock(
                        ...
                        metadata={'condition': self._extract_condition(blocks[start_idx])},
                    )
    ...
    def _try_match_for(self, blocks: list[ControlBlock], start_idx: int,
        ...
        for inc_idx in range(start_idx + 2, min(start_idx + 10, len(blocks))):
            ...
            if (self._has_assignment(inc_block) and 
                inc_block.instructions[-1].opcode_name in ['JUMP', 'JMP']):
    ...
    def _try_match_repeat_until(self, blocks: list[ControlBlock], start_idx: int,
                                processed: set[int]) -> ControlBlock | None:
