...
class DataWindowExtractor:
    ...
    def _extract_to_end(data: bytes, syntax_pos: int) -> str | None:
        ...
        try:
            ...
            cleaned = decoded.replace('\ufffd', '')  # Remove Unicode replacement character
            
            # Additional cleanup for common corruption patterns
            import re
            # Remove sequences that look like binary data interpreted as text
            cleaned = re.sub(r'[\u4000-\u9fff\u3400-\u4dbf]+', '', cleaned)  # Remove CJK characters
            cleaned = re.sub(r'䅄⩔\w+Ƕ', '', cleaned)  # Remove specific corruption pattern
    ...
    def _extract_with_segments(data: bytes, syntax_pos: int) -> str | None:
                ...
                try:
                    ...
                    if ord(char) < 32 and char not in '\r\n\t':
                        ...
                        if i + 8 < len(raw_data):
                            ...
                            if (next_bytes[0] > 127 and next_bytes[2] > 127 and 
                                next_bytes[4] > 127 and next_bytes[6] > 127):
                                # Likely metadata - skip ahead
                                # Look for next valid text
                                j = i + 2
                                while j < len(raw_data) - 1:
                                    try:
                                        test_char = raw_data[j:j+2].decode('utf-16-le', errors='strict')
        ...
        result = re.sub(r'([a-z])([A-Z])', r'\1_\2', result)  # Fix camelCase that got concatenated
        
        # Specific fixes for known patterns
        result = result.replace('chart_account_type', 'chart_of_accounts.chart_account_type')
        ...
        while i < len(text):
            ...
            if 32 <= ord(char) < 127 or char in '\r\n\t':
                ...
                if i > 0 and i < len(text) - 1:
                    ...
                    if (prev_char.isalnum() and next_char.isalnum() and 
                        chars and chars[-1] not in ' \r\n\t'):
                        ...
                        known_words = ['account', 'linkedaccount', 'description', 'column', 
