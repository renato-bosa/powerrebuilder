...
class OutputValidator:
    ...
    def _validate_block_structure(self, lines: list[str]) -> None:
        ...
        for i, line in enumerate(lines, 1):
            ...
            for start_keyword in self.BLOCK_START_KEYWORDS:
                ...
                if start_keyword == "do":
                    ...
                    if stripped == "do" or stripped.startswith("do while") or stripped.startswith("do until"):
                        if "while" in stripped:
                            block_stack.append(("do while", i))
                        else:
                            block_stack.append(("do", i))
                        break
                elif stripped == start_keyword or stripped.startswith(start_keyword + " "):
                    ...
                    if stripped == "loop" or stripped.startswith("loop "):
                        ...
                        if not block_stack:
                            self.errors.append(ValidationError(
                                i, f"Unexpected '{stripped}' without matching block start"
                            ))
                        else:
                            expected_start = None
                            ...
                            if expected_start:
                                # Pop matching blocks
                                if block_stack and (block_stack[-1][0] == expected_start or block_stack[-1][0] == "do"):
                                    block_stack.pop()
                                else:
                                    # Mismatched block
                                    if block_stack:
                                        ...
                                        self.errors.append(ValidationError(
                                            i, f"Expected 'end {actual_start}' but found '{stripped}' (started at line {start_line})"
                                        ))
                                    else:
                                        self.errors.append(ValidationError(
                                            i, f"Unexpected '{stripped}'"
                                        ))
                        break
                elif stripped.startswith(end_keyword) or stripped == end_keyword:
                    if not block_stack:
                        self.errors.append(ValidationError(
                            i, f"Unexpected '{end_keyword}' without matching block start"
                        ))
                    else:
                        expected_start = None
                        for start, end in self.BLOCK_PAIRS.items():
                        ...
                        if expected_start:
                            ...
                            if block_stack and block_stack[-1][0] == expected_start:
                                ...
                                if block_stack:
                                    actual_start, start_line = block_stack[-1]
                                    self.errors.append(ValidationError(
                                        i, f"Expected 'end {actual_start}' but found '{end_keyword}' (started at line {start_line})"
                                    ))
                                else:
                                    self.errors.append(ValidationError(
                                        i, f"Unexpected '{end_keyword}'"
                                    ))
                    break
        ...
        while block_stack:
            ...
            self.errors.append(ValidationError(
                len(lines), f"Unclosed '{block_type}' block started at line {start_line}"
            ))
    ...
    def _validate_syntax_patterns(self, lines: list[str]) -> None:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if any(keyword in stripped.lower() for keyword in ["function", "subroutine"]):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if var_match:
                ...
                if not self.IDENTIFIER_PATTERN.match(identifier):
                    self.errors.append(ValidationError(
                        i, f"Invalid identifier: '{identifier}'"
                    ))
