...
def fix_duplicate_statements(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if i + 2 < len(lines):
            ...
            if (next_line == "" and 
                third_line.startswith(current_stripped) and 
                ':' in third_line[len(current_stripped):]):
        ...
        if i + 1 < len(lines):
            ...
            if (next_line.startswith(current_stripped) and 
                ':' in next_line[len(current_stripped):]):
                fixed_lines.append(line)
                i += 2
                continue
...
def fix_unexpected_indent(content: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if i > 0:
            ...
            elif prev_line in ('pass', 'continue', 'break', 'return') or prev_line.startswith('return '):
...
def main():
    ...
    for py_file in src_dir.rglob('*.py'):
        ...
        if not has_error:
            ...
            print(f"  {py_file}: {error_msg.split('(')[0].strip()}")
    ...
    if remaining_errors:
        print(f"\n{len(remaining_errors)} files still have syntax errors:")
        for filepath, error_msg in remaining_errors:
            print(f"  {filepath}: {error_msg.split('(')[0].strip()}")
