...
def demo_error_context():
    ...
    try:
        with error_handler("generate", "generating code") as outer_ctx:
            logger.info("Processing application...")
            
            with error_handler("generate", "processing window", file_path=Path("w_main.srw")) as inner_ctx:
...
def demo_error_collection():
    ...
    # Add some warnings
    collector.add_warning("Deprecated function usage", 
                         ErrorContext(stage="parse", operation="analysis"))
    collector.add_warning("Unused variable 'temp'",
                         ErrorContext(stage="parse", operation="analysis", 
                                    file_path=Path("file1.sru"), line_number=50))
...
def demo_recovery_strategies():
    ...
    try:
        with error_handler("extract", "reading file") as ctx:
            # Simulate encoding error
            data = b'\x80\x81\x82'.decode('utf-8')
    ...
    try:
        with error_handler("parse", "loading configuration") as ctx:
            # Try to read non-existent file
            content = Path("missing_config.ini").read_text()
    except FileNotFoundError:
        # Use custom recovery
        content = handler.handle_missing_file("missing_config.ini")
...
def demo_exception_chains():
    ...
    try:
        ...
        print(format_exception_chain(e, include_traceback=False))
...
def demo_stage_specific_handling():
    ...
    def extract_with_recovery(pbl_path: Path) -> List[str]:
        ...
        try:
            with error_handler("extract", "standard extraction", file_path=pbl_path, collector=collector):
                # Simulate extraction
            ...
            try:
                with error_handler("extract", "recovery extraction", file_path=pbl_path, collector=collector):
                    # Simulate recovery
        ...
        if collector.has_errors():
            logger.warning("Extraction completed with %d errors", collector.get_error_count())
    ...
    def parse_with_error_limit(source_files: List[Path], max_errors: int = 10):
        ...
        for source_file in source_files:
            try:
                with error_handler("parse", f"parsing {source_file.name}", 
                                 file_path=source_file, collector=collector):
                    # Simulate parsing with some errors
                    if "error" in source_file.name:
                        raise ParseError(f"Syntax error in {source_file.name}", 
                                       filename=str(source_file), line=10)
...
def demo_custom_error_handlers():
    ...
    from src.common.error_handling import BaseErrorHandler
    
    class BusinessRuleHandler(BaseErrorHandler):
        """Handle business rule violations."""
        
        def can_handle(self, error: Exception) -> bool:
            return isinstance(error, ValidationError) and "business rule" in str(error).lower()
        ...
        def handle(self, error: Exception, context: ErrorContext) -> Any:
            ...
            elif "quantity" in context.operation:
