        ...
        for pcode_file in pcode_files[:3]:
            try:
                ...
                result['samples'].append({
                    ...
                    'has_code': 'function' in decompiled.lower() or 'type' in decompiled.lower()
                })
                
            except Exception as e:
                result['errors'].append(f"{pcode_file.name}: {str(e)}")
                
        result['status'] = 'success' if result['files'] > 0 else 'failed'
        
    except Exception as e:
        result['errors'].append(str(e))
        logger.error(f"Decompilation setup failed: {e}")
...
def main():
        ...
        if extraction_result['status'] == 'success':
            ...
            if decompilation_result['status'] == 'success':
                ...
                for sample in decompilation_result['samples']:
                    logger.info(f"  - {sample['name']}: {sample['lines']} lines, has_code={sample['has_code']}")
    ...
    for pbd in results['pbd_files']:
        print(f"\nPBD: {pbd['name']}")
        print(f"  Extraction: {pbd['extraction']['status']} ({pbd['extraction']['files']} files)")
        if pbd['extraction']['file_types']:
            print(f"    Types: {pbd['extraction']['file_types']}")
        print(f"  Decompilation: {pbd['decompilation']['status']} ({pbd['decompilation']['files']} files)")
        if pbd['decompilation']['errors']:
            print(f"    Errors: {len(pbd['decompilation']['errors'])}")
