    ...
    try:
        ...
        for file_info in extracted_files[:5]:  # Test first 5 files
            try:
                ...
                if file_path.exists() and file_path.suffix in ['.sru', '.srw', '.srf', '.srm']:
                ...
                result['errors'].append(f"Failed to parse {file_info['name']}: {str(e)}")
                
        if result['objects_parsed'] > 0:
            result['status'] = 'partial' if result['errors'] else 'success'
            
    except Exception as e:
        result['errors'].append(f"Parsing setup failed: {str(e)}")
...
def test_decompilation_module(extracted_files: list, output_dir: Path) -> dict:
    ...
    try:
        ...
        pcode_files = [f for f in extracted_files if Path(f['path']).suffix in ['.fun', '.win', '.udo']]
        ...
        for file_info in pcode_files[:5]:  # Test first 5 P-code files
            try:
                ...
                result['errors'].append(f"Failed to decompile {file_info['name']}: {str(e)}")
                
        if result['objects_decompiled'] > 0:
            result['status'] = 'partial' if result['errors'] else 'success'
            
    except Exception as e:
        result['errors'].append(f"Decompilation setup failed: {str(e)}")
        logger.error(f"Decompilation error: {e}", exc_info=True)
        
    return result


def test_generation_module(parsed_objects: list, decompiled_objects: list, output_dir: Path) -> dict:
...
class GeneratedClass:
    def __init__(self):
        pass
"""
                    model_path = generated_dir / f"generated_{Path(obj['name']).stem}.py"
                    ...
                    result['errors'].append(f"Failed to generate from {obj['name']}: {str(e)}")
        ...
        result['errors'].append(f"Generation setup failed: {str(e)}")
...
def run_full_pipeline_test():
    ...
    output_dir = Path("output/pipeline_test_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    for pbd_path in pbd_files:
        ...
        if extraction_result['status'] == 'success':
            ...
            results.summary['total_objects_extracted'] += extraction_result['objects_extracted']
            
            # Phase 2: Parsing (on extracted source files)
            logger.info("\nPhase 2: PARSING")
            parsing_result = test_parsing_module(extraction_result['objects'], output_dir)
            ...
            if parsing_result['objects_parsed'] > 0:
                ...
                results.summary['total_objects_parsed'] += parsing_result['objects_parsed']
            
            # Phase 3: Decompilation (on extracted P-code files)
            logger.info("\nPhase 3: DECOMPILATION")
            decompilation_result = test_decompilation_module(extraction_result['objects'], output_dir)
            ...
            if decompilation_result['objects_decompiled'] > 0:
                ...
                results.summary['total_objects_decompiled'] += decompilation_result['objects_decompiled']
            ...
            if generation_result['objects_generated'] > 0:
                ...
                results.summary['total_objects_generated'] += generation_result['objects_generated']
...
def generate_report(results: PipelineTestResult, output_dir: Path):
    ...
    with open(report_path, 'w') as f:
        ...
        f.write(f"Successful decompilations: {results.summary['successful_decompilations']}\n")
        f.write(f"Successful generations: {results.summary['successful_generations']}\n")
        f.write("\n")
        f.write(f"Total objects extracted: {results.summary['total_objects_extracted']}\n")
        f.write(f"Total objects parsed: {results.summary['total_objects_parsed']}\n")
        f.write(f"Total objects decompiled: {results.summary['total_objects_decompiled']}\n")
        f.write(f"Total objects generated: {results.summary['total_objects_generated']}\n")
        f.write("\n")
        ...
        for pbd_name in results.extraction_results:
            ...
            if pbd_name in results.decompilation_results:
                ...
                f.write(f"  Objects decompiled: {decomp_result['objects_decompiled']}\n")
...
if __name__ == "__main__":
    try:
        ...
        print("="*60)
        print(f"Extraction success rate: {results.summary['successful_extractions']}/{results.summary['total_pbd_files']}")
        print(f"Total objects extracted: {results.summary['total_objects_extracted']}")
        print(f"Total objects parsed: {results.summary['total_objects_parsed']}")
        print(f"Total objects decompiled: {results.summary['total_objects_decompiled']}")
        print(f"Total objects generated: {results.summary['total_objects_generated']}")
