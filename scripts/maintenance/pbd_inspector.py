...
def hex_dump(data: bytes, offset: int = 0, bytes_per_line: int = 16, 
    ...
    for i in range(0, len(data), bytes_per_line):
        ...
        if len(chunk) < bytes_per_line:
            ...
            if len(chunk) <= 8:
                padding = '   ' * missing + '  '  # Extra spaces for missing middle separator
            else:
                padding = '   ' * missing
            hex_values += padding
...
def check_file_signature(file_path: Path) -> Tuple[Optional[str], bytes]:
...
def inspect_pbd_structure(file_path: Path, block_size: int = DEFAULT_BLOCK_SIZE) -> None:
    ...
    """
    print(f"\n=== PowerBuilder File Structure Analysis ===")
    ...
    print(hex_dump(header, 0, 16))
    
    if sig_desc.startswith("PowerBuilder"):
        print(f"\nAnalyzing with block size: {block_size}")
        ...
        try:
            ...
            for block_num in range(4):
                ...
                print(f"\nBlock {block_num} at offset 0x{offset:08x} ({sig_desc}):")
                print(hex_dump(data, offset, 16))
...
""")
    ...
    parser.add_argument('-l', '--length', type=lambda x: int(x, 0), default=DEFAULT_LENGTH,
                        help='Number of bytes to display (default: %(default)s)')
    parser.add_argument('-b', '--bytes-per-line', type=int, default=DEFAULT_BYTES_PER_LINE,
    ...
    parser.add_argument('--inspect', action='store_true',
                        help='Perform PowerBuilder structure analysis')
    parser.add_argument('--block-size', type=int, default=DEFAULT_BLOCK_SIZE,
                        help='Block size for PBD analysis (default: %(default)s)')
    parser.add_argument('--list-files', nargs='*', dest='files',
        ...
        try:
            if args.inspect:
                ...
                if args.offset >= file_size:
                    print(f"Error: Offset {args.offset} exceeds file size {file_size}")
                    continue
                    
                # Calculate actual length to read
                length = min(args.length, file_size - args.offset) if args.length else file_size - args.offset
