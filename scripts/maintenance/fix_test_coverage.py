    ...
    def analyze_import_errors(self) -> Dict[str, List[str]]:
        ...
        for test_file in test_files:
            ...
            result = subprocess.run(
                [sys.executable, "-c", f"import sys; sys.path.insert(0, '{self.project_root}'); import {test_file.stem}"],
            ...
            if result.returncode != 0 and "ImportError" in result.stderr:
                errors.append(f"Import error: {result.stderr.split('ImportError:')[1].strip()}")
        ...
        for test_file in test_files:
            ...
            if content != original_content:
                if not dry_run:
                    ...
                    print(f"Fixed imports in: {test_file.relative_to(self.project_root)}")
                else:
                    print(f"Would fix imports in: {test_file.relative_to(self.project_root)}")
                fixed_count += 1
...
'''
        ...
        print(f"\nCreated test templates in: {templates_dir}")
...
def main():
    ...
    parser.add_argument("--run-tests", action="store_true", help="Run tests incrementally")
    parser.add_argument("--coverage", action="store_true", help="Generate coverage report")
    parser.add_argument("--strategy", action="store_true", help="Generate test improvement strategy")
    parser.add_argument("--create-templates", action="store_true", help="Create sample test templates")
    ...
    if args.fix_imports or args.all:
        ...
        fixed = fixer.fix_import_errors(dry_run=args.dry_run)
        print(f"Fixed {fixed} files")
        
    if args.run_tests:
        print("\n=== Running Tests Incrementally ===")
        results = fixer.run_tests_incrementally()
        print(f"\nSummary:")
    ...
    if args.coverage or args.all:
        print("\n=== Generating Coverage Report ===")
        coverage = fixer.generate_coverage_report()
