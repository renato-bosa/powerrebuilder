...
from src.common.performance import get_performance_monitor, log_system_info, monitor_performance
...
class PowerRebuilderBenchmark:
    ...
    def run_benchmark(self, benchmark_config: dict) -> dict:
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.error("Benchmark %s failed after %.2f seconds: %s", name, duration, e)
    ...
    def _analyze_results(self):
        ...
        completed_benchmarks = [b for b in self.results["benchmarks"] if b["status"] == "completed"]
        ...
        # Find baseline
        baseline = next((b for b in completed_benchmarks if b["name"] == "baseline_sequential"), None)
    ...
    def _generate_insights(self, benchmarks: list):
        ...
        # Cache effectiveness
        cache_benchmarks = [b for b in benchmarks if b.get("result", {}).get("cache_enabled")]
        if cache_benchmarks:
            ...
            if avg_cache_hit_rate > 50:
                insights.append(f"Caching is highly effective with {avg_cache_hit_rate:.1f}% average hit rate")
            elif avg_cache_hit_rate > 20:
                insights.append(f"Caching shows moderate effectiveness with {avg_cache_hit_rate:.1f}% hit rate")
            else:
                insights.append(f"Caching has low effectiveness with {avg_cache_hit_rate:.1f}% hit rate - check cache configuration")
        
        # Parallel processing effectiveness
        parallel_benchmarks = [b for b in benchmarks if b.get("config", {}).get("enable_parallel")]
        sequential_benchmarks = [b for b in benchmarks if not b.get("config", {}).get("enable_parallel")]
        
        if parallel_benchmarks and sequential_benchmarks:
            avg_parallel_time = sum(b["duration"] for b in parallel_benchmarks) / len(parallel_benchmarks)
            avg_sequential_time = sum(b["duration"] for b in sequential_benchmarks) / len(sequential_benchmarks)
            
            if avg_parallel_time < avg_sequential_time * 0.7:
                insights.append("Parallel processing provides significant performance benefits")
            elif avg_parallel_time < avg_sequential_time * 0.9:
                insights.append("Parallel processing provides moderate performance benefits")
            else:
                insights.append("Parallel processing shows limited benefits - may be I/O bound")
        ...
        with summary_file.open("w") as f:
            ...
            for benchmark in self.results["benchmarks"]:
                ...
                if benchmark["status"] == "completed":
                    ...
                    if result.get("cache_enabled"):
                        f.write(f"Cache hit rate: {result.get('cache_hit_rate', 'N/A')}\n")
...
def main():
    ...
    parser.add_argument("input_dir", type=Path, help="Directory containing P-code files to benchmark")
    parser.add_argument("--output-dir", type=Path, default=None, help="Output directory for benchmark results")
    ...
    if args.output_dir is None:
        args.output_dir = Path("benchmark_output") / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    # Print final summary
    completed_count = sum(1 for b in results["benchmarks"] if b["status"] == "completed")
