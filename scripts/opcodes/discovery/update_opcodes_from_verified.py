...
def convert_verified_to_opcodes_format(verified_opcode):
    ...
    if name.startswith("JUMP") or name in ["RETURN", "CALL", "HALT"]:
    ...
    elif name.startswith(("LOAD", "PUSH")):
        ...
        op in name for op in ["EQ", "NE", "LT", "LE", "GT", "GE", "AND", "OR", "NOT"]
    ):
...
def main() -> None:
    ...
    for hex_key, verified_opcode in verified_data.get("opcodes", {}).items():
        ...
        try:
            opcode_num = int(hex_key, 16) if hex_key.startswith("0x") else int(hex_key)
        except ValueError:
            continue
