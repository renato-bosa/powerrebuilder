...
def apply_renames(resolution_plan_path) -> None:
...
def update_imports(renames) -> None:
    ...
    for rename in renames:
        ...
        module_path = module_path.removeprefix("src.")  # Remove 'src.' prefix
                ...
                try:
                    ...
                    for module, name_map in rename_map.items():
                        for old_name, new_name in name_map.items():
                            ...
                            pattern3 = (
                                rf"\b{re.escape(module_alias)}\.{re.escape(old_name)}\b"
                            )
                            content = re.sub(
                                pattern3, f"{module_alias}.{new_name}", content
                            )
