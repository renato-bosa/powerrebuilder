...
from src.contracts.generators import IUIProcessor
...
class UIProcessor(IUIProcessor):
    ...
    def generate_layout(self, controls: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        layout_generator = self._layout_strategies.get(
            layout_type, 
            self._generate_absolute_layout
        )
    ...
    def extract_menus(self, window: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if 'controls' in window:
            for control in window['controls']:
                if control.get('type', '').lower() in ['menu', 'menubar', 'popupmenu']:
    ...
    def _requires_state(self, control_type: str) -> bool:
        ...
        stateful_controls = {
            'singlelineedit', 'multilineedit', 'checkbox', 'radiobutton',
            'dropdownlistbox', 'listbox', 'datawindow', 'tab', 'treeview',
            'datepicker', 'monthcalendar', 'progressbar', 'hscrollbar', 
            'vscrollbar'
        }
        
        return control_type in stateful_controls
    
    def _process_properties(self, properties: Dict[str, Any], control_type: str) -> Dict[str, Any]:
        ...
        if control_type == 'datawindow':
            ...
            processed['scaleMode'] = properties.get('originalsize', False) ? 'none' : 'fit'
    ...
    def _determine_layout_type(self, controls: List[Dict[str, Any]]) -> str:
        ...
        if any(
            c.get('properties', {}).get('responsive', False) or
            c.get('properties', {}).get('breakpoint', '')
            for c in controls
        ):
            return 'responsive'
    ...
    def _generate_responsive_layout(self, controls: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _calculate_breakpoints(self, controls: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    def _extract_toolbar_menu(self, toolbar: Dict[str, Any]) -> Optional[Dict[str, Any]]:
