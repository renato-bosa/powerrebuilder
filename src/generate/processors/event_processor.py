...
from src.contracts.generators import IEventProcessor
...
class EventProcessor(IEventProcessor):
    ...
    def __init__(self):
        ...
        self._system_events = {
            'clicked', 'doubleclicked', 'rightclicked', 'getfocus', 
            'losefocus', 'modified', 'constructor', 'destructor',
            'open', 'close', 'activate', 'deactivate', 'timer'
        }
    ...
    def _process_single_event(self, event: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if 'parameters' in event:
            processed['parameters'] = self._process_event_parameters(event['parameters'])
    ...
    def _analyze_event_body(self, body: List[Any]) -> Dict[str, Any]:
        ...
        if 'select' in body_str or 'insert' in body_str or 'update' in body_str or 'delete' in body_str:
    ...
    def _categorize_event(self, event_name: str) -> str:
        ...
        # Lifecycle events
        if name_lower in ['constructor', 'destructor', 'open', 'close', 'create', 'destroy']:
            return 'lifecycle'
        
        # User interaction events
        if name_lower in ['clicked', 'doubleclicked', 'rightclicked', 'key', 'modified']:
        ...
        # Data events
        if any(term in name_lower for term in ['retrieve', 'update', 'item', 'row', 'data']):
            return 'data'
    ...
    def _extract_event_from_handler(self, handler_name: str) -> Optional[str]:
        """Extract event type from handler name."""
        # Common patterns
        patterns = [
