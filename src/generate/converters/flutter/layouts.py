...
class LayoutConverter:
    ...
    def convert_layout(
        ...
        if self.strategy == LayoutStrategy.INTELLIGENT:
            return self._convert_intelligent_layout(
                controls, window_width, window_height
            )
        # RESPONSIVE
        return self._convert_responsive_layout(controls, window_width, window_height)

    def _convert_absolute_layout(
        self, controls: list[dict[str, Any]], _window_width: int, _window_height: int
    ) -> str:
        ...
        sorted_controls = sorted(
            controls, key=lambda c: c.get("properties", {}).get("tab_order", 999999)
        )
    ...
    def _convert_intelligent_layout(
        self, controls: list[dict[str, Any]], _window_width: int, _window_height: int
    ) -> str:
    ...
    def _convert_responsive_layout(
        self, _controls: list[dict[str, Any]], window_width: int, window_height: int
    ) -> str:
        ...
        code += f"            final baseWidth = {window_width}.0;\n"
        code += f"            final baseHeight = {window_height}.0;\n"
    ...
    def _detect_layout_groups(
        self, positions: list[ControlPosition]
    ) -> list[LayoutGroup]:
    ...
    def _detect_aligned_groups(
        self, positions: list[ControlPosition], alignment: str
    ) -> list[list[ControlPosition]]:
        ...
        for pos in sorted_positions:
            if current_coord is None:
                ...
                current_coord = sum(coord_getter(p) for p in current_group) / len(
                    current_group
                )
            else:
                # Start new group
    ...
    def _detect_grid_patterns(
                    ...
                    for v_control in vertical_aligned:
                        ...
                        corner = next(
                            (
                                ...
                                and p not in (anchor, h_control, v_control)
                            ),
                            None,
                        )
        ...
        for grid in sorted(grid_groups, key=len, reverse=True):
            ...
            if not any(
                control.name in [c.name for g in final_grids for c in g]
                for control in grid
            ):
    ...
    def _create_group(
        self, controls: list[ControlPosition], group_type: str
    ) -> LayoutGroup:
    ...
    def _generate_intelligent_layout(
        self, groups: list[LayoutGroup], original_controls: list[dict[str, Any]]
    ) -> str:
        ...
        for i, group in enumerate(sorted_groups):
            if i > 0:
                ...
                spacing = group.bounds[1] - (
                    prev_group.bounds[1] + prev_group.bounds[3]
                )
                if spacing > 0:
    ...
    def _generate_row_layout(
        self, group: LayoutGroup, original_controls: list[dict[str, Any]]
    ) -> str:
        ...
        for i, control_pos in enumerate(sorted_controls):
            ...
            control = next(
                (c for c in original_controls if c.get("name") == control_pos.name),
                None,
            )
            if control:
    ...
    def _generate_grid_layout(
        ...
        x_positions = sorted({c.x for c in group.controls})
        y_positions = sorted({c.y for c in group.controls})
        ...
        for control_pos in group.controls:
            # Find grid indices
            col_idx = (
                x_positions.index(control_pos.x) if control_pos.x in x_positions else 0
            )
            row_idx = (
                y_positions.index(control_pos.y) if control_pos.y in y_positions else 0
            )
                ...
                if (row, col) in position_map:
                    ...
                    control = next(
                        (
                            ...
                            if c.get("name") == control_pos.name
                        ),
                        None,
                    )
                    if control:
    ...
    def _generate_free_layout(
        ...
        control = next(
            (c for c in original_controls if c.get("name") == group.controls[0].name),
            None,
        )
        if control:
    ...
    def generate_responsive_layout_methods(
        self, controls: list[dict[str, Any]], _window_width: int, _window_height: int
    ) -> str:
        ...
        if len(groups) > 1:
            ...
            for control in sorted_controls:
                ...
                code += (
                    f"              width: {size.get('width', 100)} * scale * 0.8,\n"
                )
                code += (
                    f"              height: {size.get('height', 30)} * scale * 0.8,\n"
                )
                code += f"              child: {widget},\n"
                code += "            ),\n"
            code += "          ],\n"
    ...
    def _build_control_widget(self, control: dict[str, Any]) -> str:
        ...
        if self.event_wiring_system and self.event_wirings:
            ...
            control_wirings = [
                w for w in self.event_wirings if w.control_name == control_name
            ]
            if control_wirings:
                # Use event wiring system to generate widget with events
                return self.event_wiring_system.generate_control_with_events(
                    control, control_wirings
                )
        ...
        if self.ui_converter and hasattr(self.ui_converter, "_generate_widget_code"):
            ...
            if "flutter_widget" in control and isinstance(
                control["flutter_widget"], dict
            ):
                return self.ui_converter._generate_widget_code(
                    control["flutter_widget"]
                )
        ...
        if widget_type == "Container" and control.get("type") == "rectangle":
            # Rectangle shape
            color = flutter_widget.get("flutter_properties", {}).get(
                "_fillColor", "Colors.grey"
            )
