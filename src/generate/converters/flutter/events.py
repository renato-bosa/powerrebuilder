...
class EventConverter:
    ...
    def convert_event(
        ...
        if mapping.get("callback"):
            # Regular callback
            return self._create_callback_method(
                event_name, mapping, parameters, body, control_name
            )
        # Unknown event - create generic handler
        return self._create_generic_handler(event_name, parameters, body, control_name)

    def _create_lifecycle_method(
        self, event_name: str, mapping: dict, body: list[str]
    ) -> Any:
        """Create a lifecycle method."""
        from src.model.ast.functions import Method
    ...
    def _create_callback_method(
        ...
        dart_body = self._convert_event_body(
            body, event_name, return_type, return_mapping
        )
        ...
        if return_type:
            dart_return_type = (
                "Future<bool>" if is_async and return_type == "bool" else return_type
            )
        else:
    ...
    def _create_generic_handler(
        ...
        if inferred_return_type:
            dart_return_type = (
                f"Future<{inferred_return_type}>" if is_async else inferred_return_type
            )
        else:
    ...
    def _convert_event_body(
        ...
        return_mapping: dict[int, str | None] | None = None,
    ) -> list[str]:
        ...
        for statement in body:
            ...
            # Handle common PowerBuilder patterns
                ...
                try:
                    ...
                    if "::" in stripped:
                        dart_body.append(
                            f"// Scope resolution operator not supported: {statement}"
                        )
                    elif any(
                        keyword in stripped.lower()
                        for keyword in ["goto", "halt", "yield"]
                    ):
                        dart_body.append(
                            f"// Control flow keyword not supported: {statement}"
                        )
    ...
    def _convert_return_statement(
        ...
        if return_type == "bool":
            if return_expr.lower() == "true":
                return "return true;"
            if return_expr.lower() == "false":
                return "return false;"
            if return_expr in ["1", "-1"]:
                return "return true;"
            if return_expr == "0":
                return "return false;"
        ...
        if any(
            pattern in return_expr
            for pattern in [".", "(", "IIF", "String(", "Integer(", "GetItem"]
        ):
        ...
        if return_type and return_type not in [
            ...
            try:
                ...
                if any(
                    cast in return_expr
                    for cast in ["Integer(", "String(", "Long(", "Double("]
                ):
                    return_expr = self._convert_type_cast(return_expr)
                    return f"return {return_expr};"
                converted_expr = self.expression_converter.convert_expression(
                    return_expr
                )
                ...
                if return_type.startswith("Future<"):
                    ...
                    return (
                        f"return Future.value({self._get_default_value(inner_type)});"
                    )
    ...
    def _convert_basic_return(self, statement: str, return_type: str | None) -> str:
        ...
        if return_value.lower() == "true":
            return "return true;"
        if return_value.lower() == "false":
            return "return false;"
        if return_value.lower() == "null":
            return "return null;"
    ...
    def _get_default_return(self, return_type: str | None, original_expr: str) -> str:
        ...
        if return_type == "int":
            return (
                "return 0; // Default for: " + original_expr
                if original_expr
                else "return 0;"
            )
        if return_type == "bool":
            return (
                "return false; // Default for: " + original_expr
                if original_expr
                else "return false;"
            )
        if return_type == "String":
            return (
                "return ''; // Default for: " + original_expr
                if original_expr
                else "return '';"
            )
        if return_type == "double":
            return (
                "return 0.0; // Default for: " + original_expr
                if original_expr
                else "return 0.0;"
            )
        if return_type.endswith("?"):
    ...
    def _convert_assignment_statement(self, statement: str) -> str:
            ...
            if op in statement:
                ...
                if match:
                    ...
                    try:
                        ...
                        converted_rhs = self.expression_converter.convert_expression(
                            rhs
                        )
        ...
        try:
            ...
            if len(parts) == 2:
                ...
                if "." in lhs:
                    try:
                        ...
                        if any(
                            ...
                            converted_rhs = (
                                self.expression_converter.convert_expression(rhs)
                            )
                ...
                # Check for type casting in RHS
                if any(
                    cast in rhs for cast in ["Integer(", "String(", "Long(", "Double("]
                ):
                    converted_rhs = self._convert_type_cast(rhs)
                else:
    ...
    def _needs_set_state(self, variable_name: str) -> bool:
        ...
        return bool("." not in variable_name and not variable_name.startswith("_"))
    ...
    def _convert_if_statement(self, statement: str) -> str:
        ...
        single_line_match = re.match(
            r"if\s+(.+?)\s+then\s+(.+?)(?:\s+else\s+(.+?))?\s+end\s*if",
            statement,
            re.IGNORECASE,
        )
    ...
    def _convert_complex_condition(self, condition: str) -> str:
        ...
        condition = re.sub(
            r"(.+?)\s+IS\s+NULL", r"(\1 == null)", condition, flags=re.IGNORECASE
        )
        condition = re.sub(
            r"(.+?)\s+IS\s+NOT\s+NULL", r"(\1 != null)", condition, flags=re.IGNORECASE
        )
                ...
                if method_match:
                    ...
                    if params:
                        ...
                        try:
                            ...
                            return (
                                f"{converted_object}.{method_name}({converted_params});"
                            )
                        except Exception:
    ...
    def _convert_isvalid(self, statement: str) -> str:
        ...
        if match:
            ...
            try:
                ...
                if any(
                    x in var_expr.lower() for x in ["window", "control", "datawindow"]
                ):
    ...
    def _convert_open(self, statement: str) -> str:
        ...
        if "openwithparm" in statement.lower():
            match = re.search(
                r"openwithparm\s*\(\s*([^,]+)\s*,\s*([^)]+)", statement, re.IGNORECASE
            )
            if match:
                window_name = self._to_pascal_case(match.group(1).strip().strip("\"'"))
                param = self.expression_converter.convert_expression(
                    match.group(2).strip()
                )
    ...
    def _convert_messagebox(self, statement: str) -> str:
        ...
        double_param = re.search(
            r'messagebox\s*\(\s*["\']([^"\']+)["\']\s*,\s*["\']([^"\']+)["\']\s*\)',
            statement,
            re.IGNORECASE,
        )
        single_param = re.search(
            r'messagebox\s*\(\s*["\']([^"\']+)["\']\s*\)',
            statement,
            re.IGNORECASE,
        )
  ...
  builder: (context) => AlertDialog(
    ...
    def _get_default_return(self, return_type: str | None, event_name: str) -> str:
        ...
        if return_type == "bool":
            ...
            if event_lower in ["itemchanging", "rowfocuschanging"]:
                ...
                if match:
                    ...
                    if "create" in return_expr.lower() or "new " in return_expr.lower():
                        ...
                        type_match = re.search(
                            r"(?:create|new)\s+(\w+)", return_expr, re.IGNORECASE
                        )
                        if type_match:
                            return self.type_converter.convert_type(type_match.group(1))
                    ...
                    if "(" in return_expr and ")" in return_expr:
                        ...
                        if "getitem" in return_expr.lower():
                            return "dynamic"
                        if "find" in return_expr.lower():
...
enum SqlErrorAction {
  ...
  void handleSystemError(Object error, StackTrace stackTrace) {
    ...
    def _convert_complex_return(
        self, statement: str, return_type: str | None = None
    ) -> str:
        ...
        expr = re.sub(
            iif_pattern,
            lambda m: f"({self._convert_complex_condition(m.group(1))}) ? {self.expression_converter.convert_expression(m.group(2))} : {self.expression_converter.convert_expression(m.group(3))}",
            expr,
            flags=re.IGNORECASE,
        )
    ...
    def _convert_type_cast(self, expr: str) -> str:
        ...
        cast_patterns = [
            (
                r"Integer\s*\((.+?)\)",
                lambda m: f"int.parse({self.expression_converter.convert_expression(m.group(1))}.toString())",
            ),
            (
                r"Int\s*\((.+?)\)",
                lambda m: f"int.parse({self.expression_converter.convert_expression(m.group(1))}.toString())",
            ...
            (
                r"Double\s*\((.+?)\)",
                lambda m: f"double.parse({self.expression_converter.convert_expression(m.group(1))}.toString())",
            ),
            (
                r"Dec\s*\((.+?)\)",
                lambda m: f"double.parse({self.expression_converter.convert_expression(m.group(1))}.toString())",
            ...
            (
                r"String\s*\((.+?)\)",
                lambda m: f"{self.expression_converter.convert_expression(m.group(1))}.toString()",
            ),
            (
                r"Date\s*\((.+?)\)",
                lambda m: f"DateTime.parse({self.expression_converter.convert_expression(m.group(1))})",
            ),
            (
                r"DateTime\s*\((.+?)\)",
                lambda m: f"DateTime.parse({self.expression_converter.convert_expression(m.group(1))})",
            ...
            (
                r"Boolean\s*\((.+?)\)",
                lambda m: f"({self.expression_converter.convert_expression(m.group(1))} != 0)",
            ),
            (
                r"Bool\s*\((.+?)\)",
                lambda m: f"({self.expression_converter.convert_expression(m.group(1))} != 0)",
    ...
    def _convert_string_concat(self, expr: str) -> str:
        ...
        if len(parts) > 1:
            ...
            for part in parts:
                ...
                # Expression
                elif part.lower().startswith("string("):
                    # String() function call
                    match = re.match(r"string\s*\((.+)\)", part, re.IGNORECASE)
                    if match:
                        inner_expr = match.group(1)
                        try:
                            converted = self.expression_converter.convert_expression(
                                inner_expr
                            )
    ...
    def _convert_statement(self, statement: str) -> str:
        ...
        try:
            ...
            if "=" in statement:
                ...
                if len(parts) == 2:
                    try:
                        return self.expression_converter.convert_expression(
                            parts[1].strip()
                        )
                    except Exception:
