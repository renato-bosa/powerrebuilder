...
class DataWindowDefinition:
        ...
        if has_blob:
            ...
            for col in self.columns:
                if col.blob_metadata:
                    ...
                    if widget_name:
                        imports.append(
                            f"import '../widgets/{self._to_snake_case(widget_name)}.dart';"
                        )
    ...
    def generate_blob_handling_code(
        ...
        repository_methods = blob_converter.generate_blob_repository_methods(
            blob_fields
        )
...
class DataWindowConverter:
    ...
    def __init__(
        ...
        self.computed_field_processor = (
            computed_field_processor
            or ComputedFieldProcessor(self.expression_converter)
        )
        self.validation_rule_processor = (
            validation_rule_processor or ValidationRuleProcessor()
        )
    ...
    def convert_datawindow(self, dw_syntax: str, dw_name: str) -> DataWindowDefinition:
        ...
        definition.computed_fields = self._extract_computed_fields(
            dw_syntax, definition.columns
        )
    ...
    def _extract_sql(self, syntax: str) -> str | None:
        ...
        pbselect_match = re.search(
            r"PBSELECT\s*\((.*?)\)\s*\)", syntax, re.IGNORECASE | re.DOTALL
        )
        if pbselect_match:
    ...
    def _extract_presentation_style(self, syntax: str) -> str:
        ...
        style_match = re.search(
            r'processing\s*=\s*["\']?(\d+)["\']?', syntax, re.IGNORECASE
        )
    ...
    def _extract_property(
        self, text: str, prop: str, default: str | None = None
    ) -> str | None:
    ...
    def _extract_dropdown_values(self, col_def: str) -> list[dict[str, str]]:
        ...
        if values_match:
            ...
            for i in range(0, len(pairs), 2):
                if i + 1 < len(pairs):
                    values.append({"display": pairs[i], "data": pairs[i + 1]})
    ...
    def _extract_groups(self, syntax: str) -> list[str]:
        ...
        for _level, group_by in group_matches:
            groups.append(group_by)

        return groups

    def _extract_computed_fields(
        self, syntax: str, columns: list[DataWindowColumn]
    ) -> list[ComputedField]:
        ...
        column_dicts = [
            {"name": col.name, "data_type": col.data_type} for col in columns
        ]
    ...
    def _extract_validation_rules(
        self, columns: list[DataWindowColumn]
    ) -> list[ValidationRule]:
        ...
        try:
            ...
            if parsed_sql:
                ...
                if isinstance(stmt, SelectStatement):
                    relationships = self.relationship_extractor.extract_from_select(
                        stmt
                    )
