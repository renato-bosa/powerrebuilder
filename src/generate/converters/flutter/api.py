...
class DatabaseOperationFormatter:
    ...
    def _parse_operation(self, operation: str) -> DatabaseOperation | None:
        ...
        # INSERT operation
        if operation.upper().startswith("INSERT"):
            return self._parse_insert(operation)

        # UPDATE operation
        if operation.upper().startswith("UPDATE"):
            return self._parse_update(operation)

        # DELETE operation
        if operation.upper().startswith("DELETE"):
            return self._parse_delete(operation)

        # FETCH operation
        if operation.upper().startswith("FETCH"):
            return self._parse_fetch(operation)

        # OPEN/CLOSE cursor
        if operation.upper().startswith("OPEN"):
            return self._parse_cursor_open(operation)
        if operation.upper().startswith("CLOSE"):
            return self._parse_cursor_close(operation)

        # Transaction operations
        if operation.upper() in ["COMMIT", "ROLLBACK"]:
    ...
    def _parse_insert(self, operation: str) -> DatabaseOperation:
        ...
        match = re.search(
            r"INSERT\s+INTO\s+(\w+)\s*\(([^)]+)\)", operation, re.IGNORECASE
        )
        if match:
    ...
    def _parse_update(self, operation: str) -> DatabaseOperation:
        ...
        if match:
            ...
            set_match = re.search(
                r"SET\s+(.+?)(?:\s+WHERE|$)", operation, re.IGNORECASE
            )
            columns = []
            variables = []
            if set_match:
    ...
    def _format_single_operation(
        self, operation: DatabaseOperation, context: dict[str, Any] | None = None
    ) -> list[str]:
    ...
    def _format_flutter_operation(
        self, operation: DatabaseOperation, _context: dict[str, Any] | None = None
    ) -> list[str]:
        ...
        if operation.operation_type == "SELECT":
            if operation.variables:
                ...
                lines.append("try {")
                lines.append("  final result = await database.rawQuery(")
                ...
                lines.append("try {")
                lines.append("  final results = await database.query(")
            ...
            lines.append("try {")
            lines.append("  final id = await database.insert(")
            lines.append(f'    "{operation.table_name}",')
            lines.append("    {")
            for i, col in enumerate(operation.columns):
                var = (
                    operation.variables[i]
                    if i < len(operation.variables)
                    else f"value{i}"
                )
            ...
            lines.append("try {")
            lines.append("  final count = await database.update(")
            lines.append(f'    "{operation.table_name}",')
            lines.append("    {")
            for i, col in enumerate(operation.columns):
                var = (
            ...
            lines.append("try {")
            lines.append("  final count = await database.delete(")
            lines.append(f'    "{operation.table_name}",')
            if operation.conditions:
                lines.append(f'    where: "{operation.conditions}",')
            lines.append("  );")
    ...
    def _format_python_operation(
        ...
        if operation.operation_type == "SELECT":
            if operation.variables:
                ...
                lines.append("# Fetch single row")
                lines.append("try:")
                lines.append("    result = session.execute(")
                ...
                lines.append('    logger.error("Database error: %s", e)')
                lines.append("    return -1")
            else:
                # Multiple rows
                lines.append("# Query multiple rows")
                ...
                if (
                    ...
                    and operation.table_name != operation.sql_statement
                ):
                    # Use ORM query
                    ...
                    if operation.conditions:
                        lines.append(
                            f"    query = query.filter({operation.conditions})"
                        )
                    lines.append("    return query.all()")
                else:
                    # Raw SQL
                    lines.append("    results = session.execute(")
                    lines.append(f'        text("{operation.sql_statement}")')
                    lines.append("    ).fetchall()")
                    lines.append("    return results")
                lines.append("except Exception as e:")
                lines.append('    logger.error("Query error: %s", e)')
                lines.append("    return []")
            ...
            lines.append('    logger.error("Insert error: %s", e)')
            lines.append("    return -1")
            ...
            lines.append("except Exception as e:")
            lines.append("    session.rollback()")
            lines.append('    logger.error("Update error: %s", e)')
            lines.append("    return -1")
            ...
            lines.append("except Exception as e:")
            lines.append("    session.rollback()")
            lines.append('    logger.error("Delete error: %s", e)')
            lines.append("    return -1")
    ...
    def _format_transaction_block(
        ...
        if self.target == "flutter":
            ...
            lines.append("    return 1  # Success")
            lines.append("except Exception as e:")
            lines.append('    logger.error("Transaction error: %s", e)')
            lines.append("    return -1  # Automatic rollback")

        return lines

    def _format_cursor_operations(
        self, operations: list[str], _context: dict[str, Any] | None = None
    ) -> list[str]:
        ...
        if self.target == "flutter":
            ...
            lines.append(
                f'    result = conn.execute(text("{sql_statement or "SELECT * FROM table"}").execution_options(stream_results=True))'
            )
            lines.append("")
    ...
    def generate_repository_methods(
        self, table_name: str, operations: list[str]
    ) -> dict[str, list[str]]:
        ...
        if self.target == "flutter":
            ...
            if "SELECT" in operations:
                methods["getAll"] = self._generate_flutter_get_all(
                    table_name, model_name
                )
                methods["getById"] = self._generate_flutter_get_by_id(
                    table_name, model_name
                )

            if "INSERT" in operations:
                methods["create"] = self._generate_flutter_create(
                    table_name, model_name
                )

            if "UPDATE" in operations:
                methods["update"] = self._generate_flutter_update(
                    table_name, model_name
                )

            if "DELETE" in operations:
                methods["delete"] = self._generate_flutter_delete(
                    table_name, model_name
                )
            ...
            if "SELECT" in operations:
                methods["get_all"] = self._generate_python_get_all(
                    table_name, model_name
                )
                methods["get_by_id"] = self._generate_python_get_by_id(
                    table_name, model_name
                )
    ...
    def _generate_flutter_get_by_id(
        self, table_name: str, model_name: str
    ) -> list[str]:
    ...
    def _generate_python_get_all(self, table_name: str, model_name: str) -> list[str]:
        ...
        return [
            ...
            f'    """Get all records from {table_name} table."""',
            "    try:",
            f"        return session.query({model_name}).all()",
            "    except Exception as e:",
            f'        logger.error("Error fetching {table_name}s: %s", e)',
            "        return []",
        ]
    ...
    def _generate_python_get_by_id(self, table_name: str, model_name: str) -> list[str]:
        ...
        return [
            ...
            "    except Exception as e:",
            f'        logger.error("Error fetching {model_name} by id: %s", e)',
            "        return None",
        ]
    ...
    def _generate_python_create(self, table_name: str, model_name: str) -> list[str]:
        ...
        return [
            ...
            "    except Exception as e:",
            "        session.rollback()",
            f'        logger.error("Error creating {model_name}: %s", e)',
            "        raise",
        ]
    ...
    def _generate_python_update(self, table_name: str, model_name: str) -> list[str]:
        ...
        return [
            ...
            "    except Exception as e:",
            "        session.rollback()",
            f'        logger.error("Error updating {model_name}: %s", e)',
            "        return None",
        ]
    ...
    def _generate_python_delete(self, table_name: str, model_name: str) -> list[str]:
        ...
        return [
            ...
            "    except Exception as e:",
            "        session.rollback()",
            f'        logger.error("Error deleting {model_name}: %s", e)',
            "        return False",
        ]
