...
class UIConverter:
    ...
    def convert_control(
        ...
        if (
            control_type.lower() in ["picturebutton", "picturehyperlink"]
            and "picturename" in properties
        ):
            icon_context = {
            ...
            icon_mapping = self.design_system.convert_icon(
                properties["picturename"], icon_context
            )
    ...
    def _convert_color(self, value: Any) -> str:
        ...
        if isinstance(value, str):
            ...
            if value.lower().startswith("rgb"):
                ...
                match = re.match(
                    r"rgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)", value.lower()
                )
                if match:
                    r, g, b = match.groups()
                    return f"Color.fromRGBO({r}, {g}, {b}, 1.0)"
    ...
    def get_widget_imports(self, controls: list[dict[str, Any]]) -> list[str]:
        ...
        for control in controls:
            ...
            elif "image" in widget.lower() and control.get("properties", {}).get(
                "picturename", ""
            ).startswith("http"):
                imports.add(
    ...
    def _generate_widget_code(self, control: dict[str, Any]) -> str:
        ...
        if control.get("needs_glass_wrapper") and control.get("glassmorphic"):
            glass_lines = self.design_system.generate_glass_container(
                control, widget_code
            )
            widget_code = "\n      ".join(glass_lines)
    ...
    def _generate_base_widget_code(self, control: dict[str, Any]) -> str:
        ...
        if widget == "IconButton":
            ...
            if control.get("icon_mapping"):
                ...
                tooltip = control.get("flutter_properties", {}).get(
                    "tooltip", f"'{dart_name}'"
                )
        ...
        if widget == "LinearProgressIndicator":
        ...
        comment = (
            f" // {control_type}: {control['name']}"
            if control.get("unknown") or control.get("custom_widget")
            else ""
        )
