...
class RelationshipExtractor:
    ...
    def extract_from_select(self, select_stmt: SelectStatement) -> list[Relationship]:
        ...
        logger.debug(
            "From clause has %s joins",
            len(select_stmt.from_clause.joins) if select_stmt.from_clause.joins else 0,
        )
    ...
    def _extract_from_join(
        self, join: JoinClause, from_clause: FromClause
    ) -> Relationship | None:
        ...
        for mapping in mappings:
            ...
            logger.debug(
                "Source table: %s, Target table: %s", source_table, target_table
            )
            if mapping.source_table in [
                source_table,
                target_table,
            ] and mapping.target_table in [source_table, target_table]:
    ...
    def _extract_from_where(
        self, where_clause: WhereClause, from_clause: FromClause
    ) -> list[Relationship]:
    ...
    def _parse_join_type(self, join_str: str) -> JoinType:
        ...
        if "RIGHT" in join_upper:
            return JoinType.RIGHT
        if "FULL" in join_upper:
    ...
    def _get_table_name(self, table_ref: TableReference | Any) -> str | None:
        ...
        if isinstance(table_ref, TableReference) or hasattr(table_ref, "table_name"):
    ...
    def _extract_column_mappings(
        self, expr: Expression, alias_map: dict[str, str] | None = None
    ) -> list[ColumnMapping]:
    ...
    def _extract_column_ref(
        ...
        if isinstance(expr, ColumnReference):
            ...
            return (
                self._infer_table_from_column(expr.column_name),
                expr.column_name,
            )
    ...
    def _determine_relationship_type(
        ...
        for mapping in mappings:
            if self._is_primary_key(mapping.source_column) or self._is_primary_key(
                mapping.target_column
            ):
                has_pk_mapping = True
                break
        ...
        for mapping in mappings:
            if self._is_foreign_key(mapping.source_column) or self._is_foreign_key(
                mapping.target_column
            ):
                has_fk_mapping = True
                break
        ...
        if "_lookup" in source_table.lower() or "_lookup" in target_table.lower():
    ...
    def _is_primary_key(self, column_name: str) -> bool:
        ...
        return any(pattern.match(column_lower) for pattern in self.pk_patterns)

    def _is_foreign_key(self, column_name: str) -> bool:
        """Check if column name matches foreign key patterns."""
        column_lower = column_name.lower()
        return any(pattern.match(column_lower) for pattern in self.fk_patterns)
