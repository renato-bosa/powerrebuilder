...
from ..logic.application_converter import (
    ApplicationConverter, 
    ApplicationDefinition,
    ApplicationVariable,
    ApplicationEvent
)
from ..flutter.state.event_converter import EventConverter
from .expression_converter import ExpressionConverter
from ..flutter.ui.menu_converter import MenuConverter, MenuDefinition, MenuItem
from .type_converter import TypeConverter
from ..flutter.ui.widget_converter import UIConverter
...
class ASTConverter:
    ...
    def __init__(self) -> None:
        ...
        from ..flutter.ui.datawindow_converter import DataWindowConverter
    ...
    def _process_function(self, node: Tree) -> Method | None:
        ...
        if func_name:
            ...
            is_primitive = return_type.lower() in ["integer", "long", "decimal", "real", "double", "boolean"]
            dart_return_type = self.type_converter.convert_type(return_type, nullable=not is_primitive)

            # Check if function should be async
            is_async = self._should_be_async(body)

            return Method(
                name=self.expression_converter._to_camel_case(func_name), return_type=return_type, dart_return_type=dart_return_type, parameters=parameters, body=body, is_async=is_async, access_modifier=access_modifier,
            )
    ...
    def _process_parameter(self, node: Tree) -> Variable | None:
        ...
        for child in node.children:
            if isinstance(child, Tree):
                if child.data == "parameter_type":
                    param_type = self._get_type(child)
                elif child.data == "type":  # Also support regular type nodes
                    param_type = self._get_type(child)
        ...
        if param_type and param_name:
            ...
            is_primitive = param_type.lower() in ["integer", "long", "decimal", "real", "double", "boolean"]
            dart_type = self.type_converter.convert_type(param_type, nullable=not is_primitive)
            return Variable(
                name=param_name,
                type=param_type,
                dart_type=dart_type,
