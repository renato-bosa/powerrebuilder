...
class ExpressionConverter:
    ...
    def convert_expression(
        self, pb_expr: Any, context: dict[str, Any | None] | None = None
    ) -> str:
        ...
        return self._convert_property_access(dart_expr)
    ...
    def _convert_operators(self, expr: str) -> str:
        ...
        sorted_ops = sorted(
            self.operator_map.items(), key=lambda x: len(x[0]), reverse=True
        )
        ...
        for pb_func, dart_func in self.function_map.items():
            ...
            if dart_func.startswith("_"):
                ...
                if custom_func.startswith("."):
                    ...
                    result = re.sub(
                        pattern, custom_func + "(", result, flags=re.IGNORECASE
                    )
            else:
    ...
    def _get_custom_function(self, _pb_func: str, dart_func: str) -> str:
    ...
    def _convert_null_handling(self, expr: str) -> str:
        ...
        return re.sub(
            r"\bSetNull\s*\(\s*([^)]+)\s*\)", r"\1 = null", result, flags=re.IGNORECASE
        )
    ...
    def _to_camel_case(self, snake_str: str) -> str:
        ...
        if "." in snake_str:
            ...
            return ".".join(
                self._to_camel_case(part) if i > 0 else part
    ...
    def _convert_array_access(self, expr: str) -> str:
        ...
        return re.sub(pattern, adjust_index, result)
    ...
    def convert_assignment(self, pb_assignment: str) -> str:
        ...
        if match:
            ...
            if dart_value.lower() == "true" or dart_value.lower() == "false":
    ...
    def convert_blob_expression(self, pb_expr: str) -> str:
        ...
        def convert_blobedit(match) -> str:
            blob_var = match.group(1)
            pos = match.group(2)
            value = match.group(3)
    ...
    def get_required_blob_helpers(self) -> list[str]:
