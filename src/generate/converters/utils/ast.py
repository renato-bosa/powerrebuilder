...
class ASTConverter:
    ...
    def __init__(self) -> None:
        ...
        from src.generate.converters.logic.application import ApplicationConverter
    ...
    def _process_control(self, node: Tree) -> dict[str, Any] | None:
        ...
        if control_type and control_name:
            ...
            return self.ui_converter.convert_control(
                control_type,
                control_name,
                properties,
            )
        ...
        for child in node.children:
            if isinstance(child, Tree):
                if child.data == "event_name":
                    ...
                    # Check if event name contains control name (e.g.,
                    # "cb_ok::clicked")
    ...
    def _process_function(self, node: Tree) -> Method | None:
        ...
        if func_name:
            ...
            # unless the function explicitly returns null
            is_primitive = return_type.lower() in [
                "integer",
                "long",
                "decimal",
                "real",
            ...
            dart_return_type = self.type_converter.convert_type(
                return_type, nullable=not is_primitive
            )
            ...
            return Method(
                name=self.expression_converter._to_camel_case(func_name),
    ...
    def _process_parameter(self, node: Tree) -> Variable | None:
        ...
        for child in node.children:
            if isinstance(child, Tree):
                if child.data in {"parameter_type", "type"}:
                    param_type = self._get_type(child)
            elif isinstance(child, Token) and child.type == "IDENTIFIER":
                param_name = child.value

        if param_type and param_name:
            # Parameters are typically non-nullable unless explicitly marked
            is_primitive = param_type.lower() in [
                "integer",
                "long",
                "decimal",
                "real",
            ...
            dart_type = self.type_converter.convert_type(
                param_type, nullable=not is_primitive
            )
            return Variable(
                name=param_name,
                type=param_type,
    ...
    def _convert_return_statement(self, _node: Any) -> str:
        """Convert RETURN statement."""
        return "// RETURN statement conversion needed"

    # Other object type conversions
    def convert_user_object(self, _ast: Any) -> UserObjectDefinition:
    ...
    def _extract_datawindow_syntax(self, _ast: Any) -> str:
        """Extract DataWindow syntax from AST."""
        # This would extract the full DataWindow definition
        return ""

    def _extract_datawindow_name(self, _ast: Any) -> str:
        """Extract DataWindow name from AST."""
        # This would extract the DataWindow object name
        return "unknown_datawindow"

    def convert_menu(self, ast: Tree) -> Any:
        """Convert menu AST to MenuDefinition."""
        # Import types here to avoid circular import
        from src.generate.converters.flutter.menus import MenuDefinition
        ...
        for node in ast.children:
            if isinstance(node, Tree) and node.data == "menu_item":
                # Process menu item
    ...
    def convert_application(self, ast: Tree) -> Any:
        """Convert application AST to ApplicationDefinition."""
        # Import types here to avoid circular import
        from src.generate.converters.logic.application import ApplicationDefinition
    ...
    def _extract_menu_syntax(self, _ast: Any) -> str:
    ...
    def _convert_menu_item(self, node: Tree) -> Any | None:
        """Convert a menu_item AST node to MenuItem object."""
        # Import types here to avoid circular import
        from src.generate.converters.flutter.menus import MenuItem
    ...
    def _extract_application_properties(self, node: Tree, app_def: Any) -> None:
    ...
    def _convert_application_variable(self, node: Tree) -> Any | None:
        """Convert variable declaration AST node to ApplicationVariable."""
        # Import types here to avoid circular import
        from src.generate.converters.logic.application import ApplicationVariable
    ...
    def _convert_application_event(self, node: Tree) -> Any | None:
        """Convert event declaration AST node to ApplicationEvent."""
        # Import types here to avoid circular import
        from src.generate.converters.logic.application import ApplicationEvent
        ...
        try:
            ...
            return ApplicationEvent(name=event_name, parameters=parameters, body=body)
        except Exception as e:
    ...
    def _apply_application_property(
        self, app_def: Any, prop_name: str, prop_value: Any
    ) -> None:
