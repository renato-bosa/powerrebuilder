...
class CodegenState:
    ...
    def get_source_location(self, generated_line: int) -> SourceMapping | None:
...
class CodeGenerator:
    ...
    def _generate_array_operation(self, op: Any) -> str:
        ...
        if op.operation == "CONCAT":
            if op.arguments:
                other = (
                    op.arguments[0].name
                    if hasattr(op.arguments[0], "name")
                    else str(op.arguments[0])
                )
                return f"{array_name} + {other}"
        elif op.operation == "RESIZE" and op.arguments:
            dims = [
                str(arg.value if hasattr(arg, "value") else arg) for arg in op.arguments
            ]
    ...
    def _generate_file_operation(self, op: Any) -> str:
        ...
        if op.type == "OPEN":
            ...
            return f'Path("{op.file_path}").open("{mode}")'
        if op.type == "READ":
