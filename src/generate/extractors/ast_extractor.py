...
class ASTExtractor(IASTExtractor):
    ...
    def extract_datawindow_from_ast(self, ast: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (ast.get("node_type") == "DataWindow" or 
            ast.get("type") == "datawindow"):
            ...
            if "relationships" in ast:
                self._extract_explicit_relationships(ast["relationships"], relationships, table_name)
    ...
    def extract_methods_from_ast(self, ast: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if node_type in ["Function", "Event", "Method", "function", "event", 
                        "method", "PBFunction", "PBEvent"]:
    ...
    def _infer_foreign_key_from_column_name(self, column_name: str) -> Optional[Dict[str, str]]:
        """Infer foreign key from column name patterns."""
        patterns = [
            (r'(\w+)_id$', lambda m: {'target_table': m.group(1), 'target_column': 'id'}),
            (r'(\w+)_code$', lambda m: {'target_table': m.group(1), 'target_column': 'code'}),
            (r'(\w+)_key$', lambda m: {'target_table': m.group(1), 'target_column': 'id'}),
            (r'fk_(\w+)$', lambda m: {'target_table': m.group(1), 'target_column': 'id'}),
        ]
    ...
    def _determine_blob_usage(self, column_name: str) -> str:
        ...
        image_keywords = ['image', 'img', 'photo', 'picture', 'pic', 'thumbnail', 
                         'avatar', 'icon', 'logo', 'banner']
    ...
    def _extract_sql_relationships(
        ...
        try:
            ...
            if parsed_sql and isinstance(parsed_sql, list) and len(parsed_sql) > 0:
                ...
                sql_relationships = self.relationship_extractor.extract_from_select(sql_stmt)
                ...
                for rel in sql_relationships:
                    for mapping in rel.column_mappings:
                        ...
                        existing = any(
                            r.get("source_column") == mapping.source_column and
                            r.get("target_table") == mapping.target_table
        ...
        try:
            ...
            for col in columns:
                ...
                for table in tables_in_sql:
                    ...
                    if (self._column_matches_table(col_name, table) and 
        ...
        if from_match:
            ...
            for part in table_parts:
                ...
                if words:
                    ...
                    table_name = original_text.split(', ')[table_parts.index(part)].strip().split()[0]
