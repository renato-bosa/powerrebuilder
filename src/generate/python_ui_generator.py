...
class PythonUIGenerator(CodeGenerator):
    ...
    def __init__(self, template_dir: str, output_dir: str, ui_framework: str = "tkinter"):
    ...
    def generate_window(self, window_model: Dict[str, Any]) -> str:
        ...
        context = {
            ...
            "has_menu": any(c.get("type") == "menu" for c in window_model.get("controls", [])),
            "has_datawindow": any(c.get("type") == "datawindow" for c in window_model.get("controls", [])),
        }
        ...
        try:
            ...
            logger.error(f"Failed to generate window {window_model.get('name')}: {e}")
    ...
    def _get_required_imports(self, window_model: Dict[str, Any]) -> List[str]:
        ...
        if self.ui_framework == "tkinter":
            ...
            control_types = {c.get("type", "").lower() for c in window_model.get("controls", [])}
    ...
    def _generate_simple_window(self, context: Dict[str, Any]) -> str:
        ...
        if self.ui_framework == "tkinter":
            ...
            lines.append(f"        self.geometry('{context['width']}x{context['height']}')")
            ...
            for ctrl in context['controls']:
                ...
                if widget_type == "Button":
                    lines.append(f"        self.{name} = tk.{widget_type}(self, text='{ctrl['text']}')")
                    ...
                    lines.append(f"        self.{name} = tk.{widget_type}(self)")
                lines.append(f"        self.{name}.place(x={ctrl['x']}, y={ctrl['y']}, width={ctrl['width']}, height={ctrl['height']})")
