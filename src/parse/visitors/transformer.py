...
from src.model.constructs.global_vars import GlobalVariable, GlobalVariables
from src.model.constructs.pcode import FunctionBlock
...
from src.model.datawindow.column import PBColumn as ColumnDefinition
from src.model.datawindow.datawindow import (
...
from src.model.datawindow.table import PBTable as TableDefinition
from src.model.transaction.statement import (
    PBTransactionStatement as TransactionStatement,
)
from src.model.ui import Control, Menu, MenuItem, UserObject, Window
from src.base import PBNode

if TYPE_CHECKING:
    from src.model.transaction.transaction import (
...
TokenType = str | int | bool
ValueType = str | int | bool | list[str, dict[str, str], None]
ExpressionType = str | int | bool | list[str, dict[str, str], None]
ArgumentType = str | int | bool | list[str, dict[str, str], None]
...
class PBTransformer(Transformer, PositionMixin):
    ...
    def __init__(
        ...
        if source_text is not None:
            context = SourceContext.from_content(
                content=source_text, filename=filename or "<unknown>", )
    ...
    def HEX_INT(self, tok: Token) -> int:  

        # noqa: N802
        """Transform hexadecimal integer token."""
        return int(tok.value, 16)

    def DATE_LIT(self, tok: Token) -> str:  

        # noqa: N802
        """Transform date literal token."""
        return str(tok.value)

    def TIME_LIT(self, tok: Token) -> str:  

        # noqa: N802
        """Transform time literal token."""
        return str(tok.value)

    def CONTROL_TYPE(self, tok: Token) -> str:  

        # noqa: N802
        """Transform control type token."""
        return str(tok.value)

    def EVENT_TYPE(self, tok: Token) -> str:  

        # noqa: N802
        """Transform event type token."""
        return str(tok.value)

    def TYPE(self, tok: Token) -> str:  

        # noqa: N802
        """Transform type token."""
        return str(tok.value)

    def FROM(self, tok: Token) -> str:  

        # noqa: N802
        """Transform from token."""
        return str(tok.value)

    def WINDOW(self, tok: Token) -> str:  

        # noqa: N802
        """Transform window token."""
        return str(tok.value)

    def MENU(self, tok: Token) -> str:  

        # noqa: N802
        """Transform menu token."""
        return str(tok.value)

    def MENUITEM(self, tok: Token) -> str:  

        # noqa: N802
        """Transform menuitem token."""
        return str(tok.value)

    def SEPARATOR(self, tok: Token) -> str:  

        # noqa: N802
        """Transform separator token."""
        return str(tok.value)

    def TYPE_NAME(self, tok: Token) -> str:  

        # noqa: N802
        """Transform type name token."""
        return str(tok.value)

    def BOOLEAN(self, tok: Token) -> bool:  
    ...
    def QUESTION_MARK_PARAM(self, tok: Token) -> QuestionMarkParameter:  
        # noqa: N802
    ...
    def COLON_PARAM(self, tok: Token) -> ColonParameter:  
    ...
    # Keywords
    def GLOBAL(self, tok: Token) -> str:  
        # noqa: N802
        """Transform global keyword."""
        return str(tok.value)

    def VARIABLES(self, tok: Token) -> str:  

        # noqa: N802
        """Transform variables keyword."""
        return str(tok.value)

    def RETURN(self, tok: Token) -> str:  

        # noqa: N802
        """Transform return keyword."""
        return str(tok.value)

    def IF(self, tok: Token) -> str:  

        # noqa: N802
        """Transform if keyword."""
        return str(tok.value)

    def THEN(self, tok: Token) -> str:  

        # noqa: N802
        """Transform then keyword."""
        return str(tok.value)

    def ELSE(self, tok: Token) -> str:  

        # noqa: N802
        """Transform else keyword."""
        return str(tok.value)

    def END(self, tok: Token) -> str:  

        # noqa: N802
        """Transform end keyword."""
        return str(tok.value)

    def END_IF(self, tok: Token) -> str:  

        # noqa: N802
        """Transform end if keyword."""
        return str(tok.value)

    def TRY(self, tok: Token) -> str:  

        # noqa: N802
        """Transform try keyword."""
        return str(tok.value)

    def CATCH(self, tok: Token) -> str:  

        # noqa: N802
        """Transform catch keyword."""
        return str(tok.value)

    def FINALLY(self, tok: Token) -> str:  

        # noqa: N802
        """Transform finally keyword."""
        return str(tok.value)

    def THROW(self, tok: Token) -> str:  

        # noqa: N802
        """Transform throw keyword."""
        return str(tok.value)

    def USING(self, tok: Token) -> str:  

        # noqa: N802
        """Transform using keyword."""
        return str(tok.value)

    def TRANSACTION(self, tok: Token) -> str:  

        # noqa: N802
        """Transform transaction keyword."""
        return str(tok.value)

    def COMMIT(self, tok: Token) -> str:  

        # noqa: N802
        """Transform commit keyword."""
        return str(tok.value)

    def ROLLBACK(self, tok: Token) -> str:  

        # noqa: N802
        """Transform rollback keyword."""
        return str(tok.value)

    def CONNECT(self, tok: Token) -> str:  

        # noqa: N802
        """Transform connect keyword."""
        return str(tok.value)

    def DISCONNECT(self, tok: Token) -> str:  

        # noqa: N802
        """Transform disconnect keyword."""
        return str(tok.value)

    def LIBRARY(self, tok: Token) -> str:  

        # noqa: N802
        """Transform library keyword."""
        return str(tok.value)

    def IMPORT(self, tok: Token) -> str:  

        # noqa: N802
        """Transform import keyword."""
        return str(tok.value)

    def EXPORT(self, tok: Token) -> str:  

        # noqa: N802
        """Transform export keyword."""
        return str(tok.value)

    def SYSTEM(self, tok: Token) -> str:  

        # noqa: N802
        """Transform system keyword."""
        return str(tok.value)

    def DYNAMIC(self, tok: Token) -> str:  

        # noqa: N802
        """Transform dynamic keyword."""
        return str(tok.value)

    def INDIRECT(self, tok: Token) -> str:  
    ...
    def system_service(
        ...
        return {
            "type": "system_service", "name": str(name), "parameters": params or [], "throws": throws, "is_forward": bool(forward), }
    ...
    def library_def(
        ...
        return Library(
            name=str(name), path=Path(f"{name}.pbl"), is_system=bool(system), imports=imports, exports=exports, objects={obj.name: obj for obj in objects}, )
    ...
    def import_stmt(
        ...
        return Import(
            from_library=str(from_lib), object_name=str(object_name), )
    ...
    def export_stmt(
        ...
        return Export(
            object_name=str(object_name), to_library=str(to_lib) if to_lib else None, )
    ...
    def datawindow(
        ...
        return DataWindow(
            name=str(name), table=table, columns=columns, compute_expressions=computes, display_objects=displays, )
    ...
    def transaction_stmt(
        ...
        return TransactionStatement(
            type=str(stmt_type).upper(), transaction=trans_obj, )
    ...
    def try_catch(
        ...
        return TryCatchStatement(
            try_statements=try_block, catch_blocks=catch_blocks, finally_block=finally_block, )
    ...
    def catch_block(
        ...
        return CatchBlock(
            exception_type=exc_type, variable_name=str(var_name), statements=block, )
    ...
    def case_statement(
        ...
        for item in items:
            if isinstance(item, tuple):
                cases.append(item)
        return {
            "type": "case", "expression": expr, "cases": cases, }
    ...
    def window(
        ...
        return Window(
            name=str(name), properties=properties, controls=controls, events=events, )
    ...
    def menu(
        ...
        """
        return Menu(
            name=str(name), items=body if isinstance(body, list) else [body], )
    ...
    def menu_entry(
        ...
        return MenuItem(
            name=str(name), type="item", properties=properties, click_handler=events[0] if events else None, items=[], )
    ...
    def menu_separator(
        ...
        return MenuItem(
            name="", type="separator", properties=[], click_handler=None, items=[], )
    ...
    def var_declaration(
        ...
        return GlobalVariable(
            name=str(name), type=str(type_name), initial_value=initial_value, )
    ...
    def user_object(
        ...
        return UserObject(
            name=str(name), inherits=inherits, controls=controls, events=events, properties=properties, )
    ...
    def control(self, *args: str | list[tuple[str, str]]) -> Control:
        ...
        return Control(
            name=str(name), type=str(control_type), properties=properties, )
    ...
    def method(
        ...
        return FunctionBlock(
            name=f"{event_type}_{name}", parameters=params or [], code=code or [], )
