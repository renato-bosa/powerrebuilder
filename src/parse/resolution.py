...
class ResolutionContext:
    ...
    def add_unresolved_symbol(self, name: str) -> None:
    ...
    def add_import(self, module_name: str, alias: str | None = None) -> None:
...
class TypeResolver:
    ...
    def _register_common_array_types(self) -> None:
    ...
    def resolve_custom_type(
        ...
        if context.namespace:
            ...
            if symbol_info:
                symbol = symbol_info.ast if hasattr(symbol_info, "ast") else symbol_info
                if isinstance(symbol, dict):
                    return self._create_custom_type_from_symbol(
                        symbol, context.namespace
                    )

        return None

    def _create_custom_type_from_symbol(
        self, symbol: dict[str, Any], namespace: str | None = None
    ) -> PBCustomType:
        ...
        if base_class == "datawindow":
            custom_type = PBDataWindowType(name=type_name, namespace=namespace)
        else:
        ...
        for attr_name, attr_info in attributes.items():
            if isinstance(attr_info, dict):
                ...
                attr_type = self.type_registry.get(attr_type_name) or PBBasicType(
                    name=attr_type_name
                )
    ...
    def resolve_array_type(
        ...
        if isinstance(base_type, PBArrayType):
            ...
            return self.type_registry.create_array_type(
                base_type.element_type, all_dims
            )

        # Create array type
        return self.type_registry.create_array_type(base_type, dimensions)

    def _parse_array_syntax(self, type_name: str) -> tuple[str, list[int | None]]:
    ...
    def resolve_type_alias(
        self, alias: str, context: ResolutionContext
    ) -> PBType | None:
    ...
    def is_compatible(self, source_type: PBType, target_type: PBType) -> bool:
        ...
        if (
            isinstance(source_type, PBBasicType)
            and isinstance(target_type, PBBasicType)
            and source_type.name.lower() in numeric_types
            and target_type.name.lower() in numeric_types
        ):
    ...
    def get_context(self, file_path: Path) -> ResolutionContext | None:
