...
class TypeResolver:
    ...
    def _topological_sort(self, graph: dict[str, set[str]]) -> list[str]:
        ...
        # Kahn's algorithm
        in_degree = {node: 0 for node in all_types}
        for node in graph:
        ...
        if len(sorted_list) != len(all_types):
            self.context.errors.append("Circular dependency detected in type definitions")
    ...
    def _validate_enum(self, type_name: str, enum_type: CustomType) -> None:
        ...
        values = set()
        for field_name, field_info in enum_type.fields.items():
    ...
    def _compute_enum_type_values(self, enum_type: CustomType) -> None:
        ...
        for field_name in sorted(enum_type.fields.keys(), key=lambda x: enum_type.fields[x].get("ordinal", 0)):
            ...
            if field_info.get("value") is None:
                ...
                field_info["value"] = self._evaluate_enum_expression(field_info["value"])
