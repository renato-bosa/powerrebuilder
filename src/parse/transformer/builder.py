...
class PowerBuilderTransformer(Transformer[Any, Any]):
    ...
    def lvalue(self, items: list[Any]) -> Union[Variable, ArrayAccess]:
        ...
        for i in range(1, len(items)):
            ...
            if isinstance(suffix, list) and suffix:
                ...
                indices = []
                for item in suffix:
                    if hasattr(item, "__class__") and item.__class__.__name__ in [
                        "Expression",
                        "BinaryExpression",
                        "UnaryExpression",
                        "Literal",
                ...
                member_name = suffix.removeprefix(".")
    ...
    def if_statement(self, items: list[Any]) -> IfStatement:
        ...
        while i < len(items):
            ...
            if item_str == "elseif":
                ...
                while i < len(items):
                    next_item_str = (
                        str(items[i]).lower() if isinstance(items[i], str) else ""
                    )
        ...
        if elseif_branches:
            ...
            current_else = (
                Block(statements=else_statements) if else_statements else None
            )
    ...
    def case_statement(self, items: list[Any]) -> CaseStatement:
        ...
        while i < len(items):
            ...
            if item_str in {"case", "of"}:
                i += 1
                continue
            if item_str == "otherwise":
                ...
                while i < len(items):
                    ...
                    i += 1
                default_body = (
                    Block(statements=default_statements) if default_statements else None
                )
            elif item_str == "end":
                break
    ...
    def case_value(self, items: list[Any]) -> dict[str, Any] | Any:
        ...
        if len(items) == 1:
            # Single value
            return items[0]
        if len(items) >= 3 and str(items[1]).lower() == "to":
