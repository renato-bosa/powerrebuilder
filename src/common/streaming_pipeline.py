...
from ..contracts.extractors import IExtractorCoordinator
from ..contracts.parsers import IParserCoordinator
from ..contracts.decompilers import IDecompilerCoordinator
from ..contracts.models import IModelCoordinator
from ..contracts.generators import IGeneratorCoordinator

logger = logging.getLogger(__name__)


class StreamingPipelineCoordinator:
    """Pipeline coordinator with in-memory streaming between stages."""
    
    def __init__(
        self,
        extract_coordinator: IExtractorCoordinator,
        decompile_coordinator: IDecompilerCoordinator,
        parse_coordinator: IParserCoordinator,
        model_coordinator: IModelCoordinator,
        generate_coordinator: IGeneratorCoordinator,
    ...
    def run_pipeline(
        ...
        logger.info(
            ...
            input_path, output_path, use_streaming
        )
    ...
    def _run_extract_stage(
        ...
        try:
            ...
            if input_path.suffix.lower() in ['.pbl', '.pbd']:
                ...
                # Run extraction
                extract_result = self.extract_coordinator.extract(
                    input_path, temp_dir
                )
        ...
        try:
            while not input_stream.is_closed or input_stream._queue.size > 0:
                try:
                    ...
                    pcode_stream = BytesIO(pcode_data)
    ...
    def _run_generate_stage(
        ...
        result = self.generate_coordinator.generate(
            temp_model_dir, output_path, target
        )
    ...
    def _run_file_based_pipeline(
        ...
        stages = [
            ...
            ("decompile", self.decompile_coordinator.decompile, extract_dir, decompile_dir),
            ("parse", self.parse_coordinator.parse, decompile_dir, parse_dir),
            ("model", self.model_coordinator.build_models, parse_dir, model_dir),
            ("generate", self.generate_coordinator.generate, model_dir, generate_dir, target)
        ]
...
class AsyncStreamingPipeline:
    ...
    async def run_pipeline(
        ...
        streams = {
            ...
            "parse_to_model": AsyncMemoryStream(
                "parse", "model", "ast", 100
            ),
            "model_to_generate": AsyncMemoryStream(
                "model", "generate", "model", 100
            )
        }
        ...
        tasks = [
            ...
            self._run_decompile_async(
                streams["extract_to_decompile"],
                streams["decompile_to_parse"]
            ),
            self._run_parse_async(
                streams["decompile_to_parse"],
                streams["parse_to_model"]
            ),
            self._run_model_async(
                streams["parse_to_model"],
                streams["model_to_generate"]
            ),
            self._run_generate_async(
                streams["model_to_generate"],
                output_path,
                target
            )
        ...
        for stage, result in zip(stats["stages"], results):
    ...
    async def _run_extract_async(
        self,
        input_path: Path,
        output_stream: AsyncMemoryStream
    ) -> Dict[str, Any]:
    ...
    async def _run_decompile_async(
        self,
        input_stream: AsyncMemoryStream,
        output_stream: AsyncMemoryStream
    ) -> Dict[str, Any]:
    ...
    async def _async_decompile(self, pcode_item: Dict[str, Any]) -> Dict[str, Any]:
        """Async decompile helper."""
        await asyncio.sleep(0.01)  # Simulate work
        return {"source": "decompiled source"}
    
    async def _async_parse(self, source_item: Dict[str, Any]) -> Dict[str, Any]:
        """Async parse helper."""
        await asyncio.sleep(0.01)  # Simulate work
        return {"ast": {}}
    
    async def _async_build_model(self, ast_item: Dict[str, Any]) -> Dict[str, Any]:
