...
class ResourceLimits:
    ...
    max_memory_bytes: Optional[int] = None  # Absolute memory limit
...
class ResourceMonitor:
    ...
    def check_file_size(self, size: int, filename: str = ""):
    ...
    def check_depth(self, depth: int):
    ...
    def register_file(self, size: int):
...
def with_timeout(timeout: float):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if thread.is_alive():
                raise ResourceLimitError(f"Function {func.__name__} timed out after {timeout}s")
...
def safe_read_file(path: str, max_size: Optional[int] = None) -> bytes:
    """
...
def chunked_read(path: str, chunk_size: int = 8192, max_size: Optional[int] = None):
    """
