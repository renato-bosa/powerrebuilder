...
class PerformanceMonitor:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_throughput_files_per_sec": total_files / total_duration if total_duration > 0 else 0,
            "overall_throughput_mb_per_sec": (total_bytes / (1024 * 1024)) / total_duration if total_duration > 0 else 0,
    ...
    def log_summary(self):
        ...
        for op_name, op_metrics in summary["operations"].items():
            logger.info("  %s: %.2f seconds, %d files, %.1f%% cache hit rate",
                       op_name,
...
def log_system_info():
    ...
    try:
        ...
        logger.info("CPU: %d cores @ %.1f MHz", cpu_count, cpu_freq.current if cpu_freq else 0)
        logger.info("Memory: %.1f GB total, %.1f GB available (%.1f%% used)",
                   memory.total / (1024**3),
                   memory.available / (1024**3),
                   memory.percent)
        logger.info("Disk: %.1f GB free / %.1f GB total (%.1f%% used)",
                   disk.free / (1024**3),
                   disk.total / (1024**3),
