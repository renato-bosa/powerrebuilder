...
class LRUCache:
    ...
    def __init__(self, max_size: int = 1000, max_memory: int = None):
    ...
    async def put(self, key: str, value: Any, size: Optional[int] = None):
        """Put value in cache."""
        if size is None:
            # Estimate size
    ...
    async def _evict_if_needed(self):
        ...
        while len(self._cache) > self.max_size:
            ...
            logger.debug(f"Evicted {key} (size limit)")
        ...
        while self._current_memory > self.max_memory:
            ...
            key, entry = self._cache.popitem(last=False)
            self._current_memory -= entry.size
            logger.debug(f"Evicted {key} (memory limit)")
...
def cached(cache: Union[LRUCache, FileCache], key_func: Optional[Callable] = None):
