...
class OutputDirectoryHandler:
    ...
    def _handle_existing_files(
        ...
        if self.interactive and sys.stdin.isatty():
            return self._prompt_user_confirmation(output_path, existing_files, stage_name)
        else:
    ...
    def _prompt_user_confirmation(
        ...
        click.echo(click.style(
            f"⚠️  Output directory already contains files:", 
            fg="yellow", bold=True
        ))
...
def check_and_prepare_output_directory(
    ...
    should_proceed = handler.prepare_output_directory(
        path, force_overwrite, stage_name
    )
