...
class BaseErrorHandler(ABC):
    ...
    def __init__(self, severity: ErrorSeverity = ErrorSeverity.ERROR):
...
class ErrorCollector:
    ...
    def __init__(
        ...
        self._critical_error: Optional[Exception] = None
    ...
    def get_error_count(self) -> int:
        ...
        return sum(1 for e in self.errors 
    ...
    def raise_if_errors(self) -> None:
        ...
        if error_count > 0:
            ...
            if error_count == 1 and self.errors:
                ...
                error = next(e for e in self.errors 
                           if e.severity in [ErrorSeverity.ERROR, ErrorSeverity.CRITICAL])
    ...
    def _format_error(self, error: ErrorRecord, indent: int = 0) -> str:
        ...
        if error.context.line_number:
            if error.context.column_number:
                pos_parts.append(f"line {error.context.line_number}:{error.context.column_number}")
            else:
...
class ErrorManager:
    ...
    def _handle_encoding_error(self, error: UnicodeDecodeError, context: ErrorContext) -> str:
    ...
    def _handle_memory_error(self, error: MemoryError, context: ErrorContext) -> None:
    ...
    def _handle_file_not_found(self, error: FileNotFoundError, context: ErrorContext) -> None:
