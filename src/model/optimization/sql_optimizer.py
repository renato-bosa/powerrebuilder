...
from src.model.expressions import (BinaryExpression, UnaryExpression, Literal, Expression)
from src.model.ast.nodes.base import BooleanOperation
...
class SQLOptimizer:
    ...
    def optimize(self, sql_statement: SqlStatement) -> SqlStatement:
        ...
        if isinstance(optimized, SelectStatement):
            ...
            optimized = self._optimize_set_operation(optimized)
    ...
    def _optimize_select(self, stmt: SelectStatement) -> SelectStatement:
        ...
        if stmt.from_clause:
            stmt.from_clause = self._optimize_from_clause(stmt.from_clause)
        ...
        stmt.result_columns = self._optimize_result_columns(stmt.result_columns)
    ...
    def _optimize_insert(self, stmt: InsertStatement) -> InsertStatement:
        ...
        if stmt.select_statement:
            stmt.select_statement = self._optimize_select(stmt.select_statement)
        return stmt
    ...
    def _optimize_set_operation(self, stmt: SetOperationStatement) -> SetOperationStatement:
        ...
        if stmt.right:
            if isinstance(stmt.right, SelectStatement):
                ...
                stmt.right = self._optimize_set_operation(stmt.right)

        # Convert UNION to UNION ALL if we know there are no duplicates
        if stmt.operator == "UNION" and self._can_use_union_all(stmt):
    ...
    def _optimize_unary_operation(self, op: UnaryExpression) -> Expression:
        ...
        if op.operator == "NOT" and isinstance(op.operand, UnaryExpression) and op.operand.operator == "NOT":
    ...
    def _optimize_comparison(self, comp: BinaryExpression) -> Expression:
        ...
        if isinstance(comp.left, Literal) and isinstance(comp.right, Literal):
            result = self._evaluate_comparison(comp.operator, comp.left, comp.right)
            if result is not None:
                return Literal(value=result)

        # NULL comparisons
        if comp.operator in ["=", "!=", "<>"] and isinstance(comp.right, Literal) and comp.right.value is None:
            # Convert = NULL to IS NULL, != NULL to IS NOT NULL
            if comp.operator == "=":
                return UnaryExpression(operator="IS NULL", operand=comp.left)
            else:
                return UnaryExpression(operator="IS NOT NULL", operand=comp.left)
    ...
    def _optimize_logical_operation(self, logic: BooleanOperation) -> Expression:
        ...
        if logic.operator == "AND":
            filtered = [op for op in optimized_operands if not self._is_always_true(op)]
            if not filtered:
        ...
        # Remove always-false conditions from OR
            ...
            filtered = [op for op in optimized_operands if not self._is_always_false(op)]
            if not filtered:
    ...
    def _is_always_true(self, expr: Expression) -> bool:
        ...
        if isinstance(expr, BinaryExpression) and expr.operator in ['=', '!=', '<>', '<', '>', '<=', '>=']:
            ...
            if isinstance(expr.left, Literal) and isinstance(expr.right, Literal):
                return self._evaluate_comparison(expr.operator, expr.left, expr.right) == True
        return False
