...
# ============================================================================
# String Resource Extractor (from string_extractor.py)
# ============================================================================
...
class StringResourceExtractor:
    ...
    def __init__(self) -> None:
        ...
        self.encoding_cache: dict[bytes, str | None] = {}  # Cache for encoding detection
    ...
    def extract_strings_from_data(self, data: bytes, source: str = "unknown") -> list[str]:
        ...
        primary_encoding = self._detect_encoding(data[:8192])  # Use first 8KB for detection
        ...
        if validated_strings:
            ...
            logger.info("Extracted %d strings from %s (primary encoding: %s)", 
                       len(validated_strings), source, primary_encoding)

        return sorted(validated_strings)

    def _extract_with_patterns(self, data: bytes, primary_encoding: str | None) -> set[str]:
    ...
    def _extract_null_terminated(self, data: bytes, primary_encoding: str | None) -> set[str]:
                ...
                if encoding in ['utf-16le', 'utf-16be']:
                    ...
                    while True:
                        ...
                        if len(candidate) >= 6:  # Minimum meaningful UTF-16 string
                            try:
                                decoded = candidate.decode(encoding, errors='ignore').strip()
    ...
    def _extract_length_prefixed(self, data: bytes, primary_encoding: str | None) -> set[str]:
        ...
        while offset < len(data) - 4:
            ...
            if offset + 1 < len(data):
                ...
                if 3 <= length <= 255 and offset + 1 + length <= len(data):
                    ...
                    decoded = self._decode_string_enhanced(string_data, primary_encoding)
                    if decoded:
                        strings.add(decoded)
                        offset += 1 + length
                        continue
    ...
    def _get_encoding_list(self, primary_encoding: str | None) -> list[str]:
        ...
        common_encodings = ['utf-8', 'cp1252', 'utf-16le', 'utf-16be', 'latin1', 'ascii']
    ...
    def _decode_string_enhanced(self, data: bytes, primary_encoding: str | None) -> str | None:
    ...
    def _clean_string_data(self, data: bytes) -> bytes:
        ...
        if len(data) % 2 == 0:
            ...
            elif data[::2].count(b'\x00'[0]) > len(data) // 4:
    ...
    def get_extraction_statistics(self) -> dict[str, Any]:
        ...
        success_rate = (self.extraction_stats["valid_strings"] / 
                       max(self.extraction_stats["total_candidates"], 1) * 100)

        cache_hit_rate = (self.extraction_stats["cache_hits"] / 
                         max(self.extraction_stats["encoding_detections"] + 
                             self.extraction_stats["cache_hits"], 1) * 100)
        ...
        return {
            ...
            "encoding_detections_performed": self.extraction_stats["encoding_detections"],
            "encoding_cache_hits": self.extraction_stats["cache_hits"],
            "encoding_cache_hit_rate_percent": round(cache_hit_rate, 2),
            "average_strings_per_source": round(total_strings / max(len(self.extracted_strings), 1), 2),
        }
...
class ResourceExtractionManager:
    ...
    def __init__(self, base_output_dir: Path) -> None:
        ...
        # Enhanced statistics
        self.stats = {
            "total_files_processed": 0, "files_with_resources": 0, "total_resources": 0, "unique_resources": 0, "duplicate_resources": 0, "resource_types": defaultdict(int), "resource_categories": defaultdict(int), "total_size": 0, "size_by_type": defaultdict(int), "size_by_category": defaultdict(int), "extraction_errors": 0,
        }
        ...
        # Resource management
        self.resource_registry: dict[str, dict[str, Any]] = {}  # hash -> resource metadata
        self.resource_references: dict[str, int] = defaultdict(int)  # hash -> reference count
        self.resource_access_times: dict[str, float] = {}  # hash -> last access time
    ...
    def extract_from_object(self, data: bytes, source_file: str, object_name: str, object_type: str) -> list[dict[str, Any]]:
    ...
    def extract_from_object_cached(self, data: bytes, source_file: str, object_name: str, object_type: str) -> list[dict[str, Any]]:
        ...
        cache_key = self._generate_cache_key(data, source_file, object_name, object_type)

        # Try to load from cache
        cached_resources = self._load_from_cache(cache_key)
        if cached_resources is not None:
            self.cache_stats["hits"] += 1
            logger.debug(f"Cache hit for {object_name} in {source_file}")
            return cached_resources

        # Cache miss - perform extraction
        self.cache_stats["misses"] += 1
        resources = self.extract_from_object(data, source_file, object_name, object_type)
    ...
    def get_resource_statistics(self) -> dict[str, Any]:
        ...
        most_referenced = sorted(
            self.resource_references.items(),
            key=lambda x: x[1],
            reverse=True
        )[:10]
        ...
        recently_accessed = sorted(
            self.resource_access_times.items(),
            key=lambda x: x[1],
            reverse=True
        )[:10]
        ...
        return {
            ...
            "recently_accessed": [(hash[:16], time.ctime(access_time)) for hash, access_time in recently_accessed],
        }

    def _generate_cache_key(self, data: bytes, source_file: str, object_name: str, object_type: str) -> str:
    ...
    def _cleanup_resources(self) -> None:
        ...
        target_reduction = self.current_memory_usage - (self.max_memory_usage * 0.8)  # Reduce to 80% of max
        freed_memory = 0
        removed_count = 0
        ...
        logger.info(f"Cleaned up {removed_count} resources, freed {freed_memory / 1024 / 1024:.2f} MB")
    ...
    def _generate_main_manifest(self) -> None:
        ...
        manifest = {
            "extraction_summary": {
                "total_files_processed": self.stats["total_files_processed"], "files_with_resources": self.stats["files_with_resources"], "total_resources_found": self.stats["total_resources"], "unique_resources": self.stats["unique_resources"], "duplicate_resources": self.stats["duplicate_resources"], "total_size_bytes": self.stats["total_size"], "extraction_errors": self.stats["extraction_errors"],
            }, "resource_types": dict(self.stats["resource_types"]), "resource_categories": dict(self.stats["resource_categories"]), "size_by_type": dict(self.stats["size_by_type"]), "size_by_category": dict(self.stats["size_by_category"]),
        }
    ...
    def _generate_detailed_catalog(self) -> None:
        ...
        for resource in self.all_resources:
            ...
            resource_info = {
                "id": resource["id"], "type": resource["type"], "size": resource["size"], "source_object": resource["source_object"], "source_file": resource.get("source_file", "unknown"), "path": resource["path"], "is_duplicate": resource.get("is_duplicate", False), "metadata": resource.get("metadata", {}),
            }
        ...
        catalog = {
            "by_type": dict(by_type), "by_category": dict(by_category), "by_source": dict(by_source),
        }
    ...
    def _generate_cache_report(self) -> None:
        ...
        with open(report_path, "w") as f:
            ...
            if cache_stats['hit_rate_percent'] < 50:
                f.write("- Low cache hit rate. Consider increasing cache retention time.\n")
    ...
    def _generate_resource_management_report(self) -> None:
        ...
        with open(report_path, "w") as f:
            ...
            f.write(f"  Total Resources Managed: {resource_stats['total_resources_managed']}\n")
            f.write(f"  Total References: {resource_stats['total_references']}\n")
            f.write(f"  Current Memory Usage: {resource_stats['current_memory_usage_mb']:.2f} MB\n")
            f.write(f"  Maximum Memory Limit: {resource_stats['max_memory_usage_mb']:.2f} MB\n")

            memory_usage_percent = (resource_stats['current_memory_usage_mb'] / 
                                  resource_stats['max_memory_usage_mb'] * 100)
    ...
    def cleanup(self) -> None:
        ...
        if removed > 0:
            logger.info(f"Cleaned up {removed} old cache entries during shutdown")
