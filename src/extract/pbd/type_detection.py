...
class ObjectTypeDetector:
    ...
    def _detect_from_type_code(cls, type_code: int) -> int | None:
        """Detect object type from internal type code.
        ...
        # Based on
        # reference/decompilers/powerbuilder-decompile/pbd/definitions.py
    ...
    def _detect_from_name_pattern(cls, name: str) -> int | None:
        """Detect object type from filename pattern.
    ...
    def is_binary_content(cls, data: bytes, check_length: int = 1024) -> bool:
        ...
        non_printable = sum(
            1
            for b in check_data
            if b < min_printable and b not in (tab_char, newline_char, carriage_return)
        )
        ...
        return (
            null_ratio > null_threshold or non_printable_ratio > non_printable_threshold
        )
