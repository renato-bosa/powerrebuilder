...
class PbdObject:
    ...
    # The raw_pcode attribute mentioned in the user story will be derived from
    # raw_text_content
    ...
    def _try_inflate_datawindow_syntax(self, text_content: str) -> str:
        ...
        if compression_flag == "1":
            logger.debug(
                f"Found compressed DataWindow syntax (Syntax=(1)) in {
                    self.name
            ...
            try:
                ...
                # The regex captures from a string, so we need to encode it to
                # bytes that represent the original b64 string
                ...
                try:
                    ...
                    try:
                        ...
                        logger.exception(
                            f"Failed to decode inflated DataWindow syntax for {
                                self.name
                            } with cp1252 as well. Storing as bytes repr.",
                        )
                        decompressed_syntax_str = f"<DECOMPRESSION_DECODE_ERROR: {
                            decompressed_syntax_bytes!r
                        }>"
                ...
                # Need to escape quotes in the decompressed_syntax_str for
                # embedding back into the string literal
    ...
    def __post_init__(self) -> None:
        ...
        if full_text is not None:
            ...
            length_tolerance = 0
            if self.is_partial:
                # Allow up to one block size if marked partial (BLOCK_SIZE not defined here, use a fixed value or pass it)
            ...
            if not (
                ...
                # Define the regex pattern if needed
                DW_SYNTAX_REGEX = re.compile(r'Syntax=\((\d)\)"([^"]+)"', re.DOTALL)
