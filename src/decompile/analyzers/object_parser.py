...
class PowerBuilderObject:
    ...
    def get_all_pcode_data(self) -> list[bytes]:
...
class ObjectParser:
    ...
    def _set_pcode_section(cls, obj: PowerBuilderObject, obj_data: bytes, offset: int, object_name: str) -> None:
        ...
        try:
            ...
            sections = EnhancedPCodeDetector.find_all_pcode_sections(obj_data, object_type)
            ...
            if sections:
                ...
                obj.pcode_length = (last_section.offset + last_section.length) - first_section.offset
                ...
                logger.info(
                    ...
                    len(sections), object_name, obj.pcode_offset, obj.pcode_length
                )
    ...
    def _find_pcode_section(
        ...
        """
        try:
            from src.decompile.pcode.detector import (
            ...
            if not sections:
                logger.warning("Enhanced detector found no P-code sections in %s", obj.object_name)
                return -1, 0

            # Log what was found
            logger.info(
                "Enhanced detector found %d P-code section(s) in %s:",
                len(sections), obj.object_name
            )
            for idx, section in enumerate(sections):
                logger.info(
            ...
            if len(sections) == 1:
                ...
                logger.info(
                    ...
                    first_offset, last_end, total_length
                )

                # Note: This returns the entire span including any data between sections.
