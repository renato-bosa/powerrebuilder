...
class CFGVisualizer:
    ...
    def _generate_dot(
        ...
        lines.append(
            f'  node [shape={self.options.node_shape}, fontname="{self.options.font_name}", fontsize={self.options.font_size}];'
        )
        lines.append(
            f'  edge [fontname="{self.options.font_name}", fontsize={self.options.font_size - 2}];'
        )
        lines.append("")
    ...
    def _generate_method_content(
        ...
        if blocks:
            lines.append(
                f'{prefix}entry [label="Entry", style=filled, fillcolor="{self.colors["entry"]}"];'
            )
            ...
            if i in block_graph:
                for target_idx in block_graph[i]:
                    edge_attrs = self._get_edge_attributes(
                        block, blocks[target_idx] if target_idx < len(blocks) else None
                    )
                    lines.append(
                        f"{prefix}block_{i} -> {prefix}block_{target_idx} [{edge_attrs}];"
                    )
        ...
        for i, block in enumerate(blocks):
            ...
            if block.instructions and block.instructions[-1].opcode_name in [
                "RETURN",
                "RET",
                "HALT",
                "EXIT",
        ...
        if exit_blocks:
            lines.append("")
            lines.append(
                f'{prefix}exit [label="Exit", style=filled, fillcolor="{self.colors["exit"]}"];'
            )
    ...
    def _generate_block_node(
        ...
        if self.options.show_instructions and block.instructions:
            if self.options.level == VisualizationLevel.SIMPLIFIED:
                ...
                if len(block.instructions) > max_inst:
                    label_parts.append(
                        f"... +{len(block.instructions) - max_inst} more"
                    )
        ...
        if block.then_block and self.options.show_instructions:
            ...
            if len(block.then_block.instructions) > 3:
                label_parts.append(
                    f"  ... +{len(block.then_block.instructions) - 3} more"
                )
        ...
        if block.else_block and self.options.show_instructions:
            ...
            if len(block.else_block.instructions) > 3:
                label_parts.append(
                    f"  ... +{len(block.else_block.instructions) - 3} more"
                )
    ...
    def _get_edge_attributes(
        ...
        if source_block.instructions:
            ...
            # Loop back edges
            elif target_block and target_block.start_addr < source_block.start_addr:
    ...
    def generate_summary_stats(
        ...
        stats["cyclomatic_complexity"] = (
            stats["total_edges"] - stats["total_blocks"] + 2
        )
