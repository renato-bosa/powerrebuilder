    ...
    def calculate_derived_metrics(self) -> None:
        ...
        if self.duration_seconds > 0:
            ...
            if self.total_bytes > 0:
                self.mb_per_second = (self.total_bytes / 1024 / 1024) / self.duration_seconds
        
        if self.total_files > 0:
            self.failure_rate_percent = (self.failed_files / self.total_files) * 100
        
        if self.total_bytes > 0 and self.total_files > 0:
            self.average_file_size_mb = (self.total_bytes / self.total_files) / (1024 * 1024)
...
class ComparisonResult:
    ...
    sequential_metrics: Optional[BenchmarkMetrics] = None
    parallel_basic_metrics: Optional[BenchmarkMetrics] = None
    parallel_enhanced_metrics: Optional[BenchmarkMetrics] = None
    ...
    def calculate_improvements(self) -> None:
        ...
        # Calculate speedup
        if self.parallel_basic_metrics and self.parallel_basic_metrics.duration_seconds > 0:
            self.speedup_basic = baseline_time / self.parallel_basic_metrics.duration_seconds
            workers = self.parallel_basic_metrics.worker_count or 1
            self.efficiency_basic = self.speedup_basic / workers
        
        if self.parallel_enhanced_metrics and self.parallel_enhanced_metrics.duration_seconds > 0:
            self.speedup_enhanced = baseline_time / self.parallel_enhanced_metrics.duration_seconds
        ...
        if self.parallel_enhanced_metrics:
            ...
            if baseline_failures > 0:
                self.reliability_improvement = (baseline_failures - enhanced_failures) / baseline_failures * 100
        
        # Calculate memory improvement
        if (self.parallel_basic_metrics and self.parallel_enhanced_metrics and 
            self.parallel_basic_metrics.memory_peak_mb > 0):
            basic_memory = self.parallel_basic_metrics.memory_peak_mb
            enhanced_memory = self.parallel_enhanced_metrics.memory_peak_mb
            self.memory_improvement = (basic_memory - enhanced_memory) / basic_memory * 100
...
class SystemMonitor:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            'cpu_average': statistics.mean(self.cpu_samples) if self.cpu_samples else 0.0,
            'cpu_peak': max(self.cpu_samples) if self.cpu_samples else 0.0,
            'memory_average_mb': statistics.mean(self.memory_samples) if self.memory_samples else 0.0,
...
class BenchmarkRunner:
    ...
    async def run_full_benchmark(self) -> ComparisonResult:
        ...
        if self.config.test_parallel_basic:
            ...
            result.parallel_basic_metrics = await self._benchmark_parallel_basic(test_files)
        
        # Run enhanced parallel benchmark
        if self.config.test_parallel_enhanced:
            self.console.print("\n[yellow]Running enhanced parallel benchmark...[/yellow]")
            result.parallel_enhanced_metrics = await self._benchmark_parallel_enhanced(test_files)
    ...
    async def _benchmark_parallel_basic(self, test_files: List[Path]) -> BenchmarkMetrics:
    ...
    async def _benchmark_parallel_enhanced(self, test_files: List[Path]) -> BenchmarkMetrics:
        """Benchmark enhanced parallel processing."""
        metrics = BenchmarkMetrics()
        
        # Get optimal configuration
        config = get_config()
        metrics.worker_count = config.parallelism.max_workers or (psutil.cpu_count() or 4)
        metrics.cpu_count = psutil.cpu_count()
        metrics.system_memory_gb = psutil.virtual_memory().total / (1024**3)
    ...
    def _display_results(self, result: ComparisonResult) -> None:
        ...
        rows = [
            ("Files Processed", 
             fmt(result.sequential_metrics.processed_files if result.sequential_metrics else None),
             fmt(result.parallel_basic_metrics.processed_files if result.parallel_basic_metrics else None),
             fmt(result.parallel_enhanced_metrics.processed_files if result.parallel_enhanced_metrics else None),
             ""),
            
            ("Processing Time",
             fmt(result.sequential_metrics.duration_seconds if result.sequential_metrics else None, "s"),
             fmt(result.parallel_basic_metrics.duration_seconds if result.parallel_basic_metrics else None, "s"),
             fmt(result.parallel_enhanced_metrics.duration_seconds if result.parallel_enhanced_metrics else None, "s"),
             ""),
            
            ("Speedup vs Sequential",
             "1.0x",
             f"{result.speedup_basic:.1f}x" if result.speedup_basic > 0 else "N/A",
             f"{result.speedup_enhanced:.1f}x" if result.speedup_enhanced > 0 else "N/A",
             f"+{(result.speedup_enhanced - result.speedup_basic):.1f}x" if result.speedup_enhanced > result.speedup_basic else ""),
            
            ("Files/Second",
             fmt(result.sequential_metrics.files_per_second if result.sequential_metrics else None),
             fmt(result.parallel_basic_metrics.files_per_second if result.parallel_basic_metrics else None),
             fmt(result.parallel_enhanced_metrics.files_per_second if result.parallel_enhanced_metrics else None),
             ""),
            
            ("Peak Memory",
             fmt(result.sequential_metrics.memory_peak_mb if result.sequential_metrics else None, "MB"),
             fmt(result.parallel_basic_metrics.memory_peak_mb if result.parallel_basic_metrics else None, "MB"),
             fmt(result.parallel_enhanced_metrics.memory_peak_mb if result.parallel_enhanced_metrics else None, "MB"),
             f"{result.memory_improvement:+.1f}%" if result.memory_improvement != 0 else ""),
            
            ("Failure Rate",
             fmt(result.sequential_metrics.failure_rate_percent if result.sequential_metrics else None, "%"),
             fmt(result.parallel_basic_metrics.failure_rate_percent if result.parallel_basic_metrics else None, "%"),
             fmt(result.parallel_enhanced_metrics.failure_rate_percent if result.parallel_enhanced_metrics else None, "%"),
             f"{result.reliability_improvement:+.1f}%" if result.reliability_improvement != 0 else ""),
        ]
...
• Reliability Improvement: {result.reliability_improvement:+.1f}%
    ...
    async def _save_results(self, result: ComparisonResult) -> None:
        ...
        if self.config.output_format == "json":
            output_file = self.config.output_directory / f"benchmark_results_{timestamp}.json"
            ...
            if result.parallel_enhanced_metrics:
                results_dict['parallel_enhanced'] = result.parallel_enhanced_metrics.__dict__
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="PowerRebuilder Decompilation Benchmark")
