...
from src.decompile.adaptive_parallelism import get_adaptive_engine, ParallelismConfig
...
class FileComplexityMetrics:
    ...
    def calculate_base_timeout(self) -> float:
        ...
        size_factor = max(30.0, self.file_size_bytes / (1024 * 1024) * 60)  # 1min per MB
...
class MemoryAwareTaskScheduler:
    ...
    def update_worker_stats(self, worker_id: str, memory_mb: float, cpu_percent: float) -> None:
    ...
    def _check_memory_pressure(self, worker_id: str, state: WorkerState) -> None:
        ...
        if state.memory_usage_mb > self.config.worker_memory_limit_mb:
            if not state.is_throttled:
                ...
                state.throttle_reason = f"Per-worker limit exceeded: {state.memory_usage_mb:.1f}MB"
                self._memory_stats['throttle_events'] += 1
                logger.warning("Throttling worker %s: %s", worker_id, state.throttle_reason)
        ...
        if system_memory.percent > self.config.throttle_threshold_percent:
            if not state.is_throttled:
                state.is_throttled = True
                state.throttle_reason = f"System memory pressure: {system_memory.percent:.1f}%"
                self._memory_stats['throttle_events'] += 1
                logger.warning("Throttling worker %s due to system memory pressure", worker_id)
        
        # Update peak usage
        total_worker_memory = sum(s.memory_usage_mb for s in self.worker_states.values())
        if total_worker_memory > self._memory_stats['peak_usage_mb']:
        ...
        with self._monitor_lock:
            ...
            return {
                **self._memory_stats,
                ...
                'active_workers': len([s for s in self.worker_states.values() if not s.is_throttled]),
                'throttled_workers': len([s for s in self.worker_states.values() if s.is_throttled]),
...
class HeartbeatProgressTracker:
    ...
    def start_heartbeat_monitoring(self) -> None:
        ...
        if self._heartbeat_thread is None or not self._heartbeat_thread.is_alive():
            ...
            logger.info("Started heartbeat monitoring with %ds interval", self.heartbeat_interval)
    ...
    def register_task(self, task_id: str, file_path: str, worker_id: str, 
                     estimated_timeout: float) -> None:
    ...
    def update_task_progress(self, task_id: str, progress_percent: float, 
                           intermediate_data: Optional[Dict[str, Any]] = None) -> None:
    ...
    def _check_task_timeouts(self) -> None:
        ...
        with self._lock:
            ...
            for task_id, checkpoint in self.active_tasks.items():
                ...
                if (checkpoint.estimated_completion_time and 
                    current_time > checkpoint.estimated_completion_time):
                ...
                if silence_duration > self.heartbeat_interval * 3:  # 3x heartbeat interval
                    logger.warning(
                        "Task %s silent for %.1fs (worker: %s, file: %s)",
                        task_id, silence_duration, checkpoint.worker_id, checkpoint.file_path
                    )
            
            for task_id in timed_out_tasks:
                logger.error("Task %s timed out, will be retried", task_id)
                checkpoint = self.active_tasks[task_id]
                self.fail_task(task_id, f"Timeout after {current_time - checkpoint.start_time:.1f}s")
        ...
        with self._lock:
            for task_id, checkpoint in self.active_tasks.items():
                ...
                try:
                    ...
                    logger.warning("Failed to save checkpoint for task %s: %s", task_id, e)
    ...
    def get_progress_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                ) / len(self.active_tasks) if self.active_tasks else 0.0,
...
class WorkStealingLoadBalancer:
    ...
    def get_task_for_worker(self, worker_id: str) -> Optional[Any]:
        ...
        with self._lock:
            ...
            for other_worker_id, queue in self.worker_queues.items():
                if other_worker_id != worker_id and not queue.empty():
                    if self.worker_loads[other_worker_id] > 1:  # Only steal if other worker has >1 task
                        try:
                            task = queue.get_nowait()
                            self.worker_loads[other_worker_id] -= 1
                            self.steal_events += 1
                            logger.debug("Worker %s stole task from %s", worker_id, other_worker_id)
                            return task
                        except Empty:
                            continue
            ...
            if max_load - min_load > 2:  # Significant imbalance
                ...
                if not self.worker_queues[heavy_worker].empty():
                    try:
                        ...
                        logger.debug("Rebalanced task from %s to %s", heavy_worker, light_worker)
                    except Empty:
                        pass
    ...
    def get_load_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'total_queued_tasks': sum(q.qsize() for q in self.worker_queues.values()),
                'global_queue_size': self.global_queue.qsize(),
                'steal_events': self.steal_events,
                'rebalance_events': self.rebalance_events,
                'load_imbalance': max(self.worker_loads.values()) - min(self.worker_loads.values()) 
                    if self.worker_loads else 0,
            }
...
class EnhancedParallelDecompileCoordinator(IDecompilerCoordinator):
    ...
    def __init__(
        ...
        if enable_heartbeat_tracking:
            checkpoint_path = checkpoint_dir or Path.cwd() / ".powerrebuilder_checkpoints"
        ...
        logger.info(
            ...
            self.max_workers, enable_work_stealing, enable_memory_monitoring, enable_heartbeat_tracking
        )
    ...
    def decompile(
        ...
        try:
            ...
            if enable_resumption and self.heartbeat_tracker:
                ...
                if checkpoints:
                    logger.info("Found %d checkpoint(s) for resumption", len(checkpoints))
                    # Filter out files that were already being processed
                    checkpoint_files = {cp.file_path for cp in checkpoints}
                    files_to_process = [f for f in files_to_process if str(f) not in checkpoint_files]
            ...
            if parallelism_config.use_parallelism:
                ...
                logger.info("Adaptive parallelism: workers=%d, processes=%s", 
                           self.max_workers, parallelism_config.use_processes)
            ...
            result.update({
                'performance': {
                    ...
                    'speedup_estimate': self._calculate_speedup_estimate(duration, self.stats['total_files']),
                },
                'memory_stats': self.memory_scheduler.get_memory_stats() if self.memory_scheduler else {},
                'load_balancer_stats': self.load_balancer.get_load_stats() if self.load_balancer else {},
                'progress_stats': self.heartbeat_tracker.get_progress_summary() if self.heartbeat_tracker else {},
                'adaptive_config': {
            ...
            logger.info("Processed %d/%d files (%.1f%% success rate)", 
                ...
                with file_path.open('rb') as f:
                    ...
                    if sample:
                        ...
                        complexity.has_complex_structures = (
                            b'HDR' in sample or b'NOD' in sample or 
                            sample.count(0x00) > len(sample) * 0.3
                        )
    ...
    def _process_with_processes(
        ...
        config: ParallelismConfig, progress_callback: Optional[Callable]
    ) -> Dict[str, Any]:
        ...
        with Live(self._create_progress_layout(progress), console=console, refresh_per_second=4):
                ...
                for future in as_completed(future_to_file):
                    ...
                    try:
                        ...
                        if result['success']:
                            ...
                            if self.heartbeat_tracker:
                                self.heartbeat_tracker.fail_task(task_id, result.get('error', 'Unknown'))
                        ...
                        if progress_callback:
                            progress_callback(completed, len(files), f"Completed {file_path.name}")
    ...
    def _create_progress_layout(self, progress: Progress) -> Panel:
        ...
        return Panel(progress, title="Enhanced Decompilation Progress", border_style="blue")
    ...
    def _create_result(self, status: str, error: Optional[str] = None) -> Dict[str, Any]:
    ...
    def decompile_file(self, file_path: Path) -> str:
        ...
        if temp_results['status'] == 'completed':
            ...
            raise RuntimeError(f"Failed to process {file_path.name}: {temp_results.get('error', 'Unknown')}")
...
def _enhanced_process_file_worker(
    ...
    logger.info("Worker %d processing %s (timeout: %.1fs)", worker_pid, file_path.name, timeout)
