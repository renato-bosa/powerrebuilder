...
class TimeoutConfig:
    ...
    type_multipliers: dict[str, float] = field(
...
class DecompilationConfig:
    ...
    def validate(self) -> list[str]:
        ...
        if (
            self.memory.throttle_threshold_percent
            >= self.memory.max_system_memory_percent
        ):
            issues.append("Throttle threshold must be less than max memory percent")

        # Validate parallelism settings
        if (
            self.parallelism.max_workers is not None
            and self.parallelism.max_workers < 1
        ):
