...
from .enhanced_stack import StackValue, StackValueType, StackValueOrigin
...
class TypeInferenceEngine:
    ...
    def __init__(self) -> None:
        ...
        self.pb_type_patterns = {
            ...
            re.compile(r'.*_str$|.*_string$|.*text.*', re.IGNORECASE): StackValueType.STRING,
            re.compile(r'.*_int$|.*_integer$|.*count.*|.*index.*', re.IGNORECASE): StackValueType.INTEGER,
            re.compile(r'.*_bool$|.*_boolean$|.*flag.*|is_.*', re.IGNORECASE): StackValueType.BOOLEAN,
            re.compile(r'.*_date$|.*_dt$|.*date.*', re.IGNORECASE): StackValueType.DATE,
            re.compile(r'.*_time$|.*time.*', re.IGNORECASE): StackValueType.TIME,
            re.compile(r'.*_real$|.*_double$|.*amount.*|.*price.*', re.IGNORECASE): StackValueType.REAL,
        }
        
        # Common PowerBuilder method signatures
        self.pb_method_signatures = {
            'messagebox': MethodInfo('MessageBox', StackValueType.INTEGER, [StackValueType.STRING, StackValueType.STRING], 2),
            'isnull': MethodInfo('IsNull', StackValueType.BOOLEAN, [StackValueType.UNKNOWN], 1),
            'len': MethodInfo('Len', StackValueType.INTEGER, [StackValueType.STRING], 1),
            'mid': MethodInfo('Mid', StackValueType.STRING, [StackValueType.STRING, StackValueType.INTEGER, StackValueType.INTEGER], 3),
            'upper': MethodInfo('Upper', StackValueType.STRING, [StackValueType.STRING], 1),
            'lower': MethodInfo('Lower', StackValueType.STRING, [StackValueType.STRING], 1),
            'trim': MethodInfo('Trim', StackValueType.STRING, [StackValueType.STRING], 1),
            'string': MethodInfo('String', StackValueType.STRING, [StackValueType.UNKNOWN], 1),
            'integer': MethodInfo('Integer', StackValueType.INTEGER, [StackValueType.UNKNOWN], 1),
            'real': MethodInfo('Real', StackValueType.REAL, [StackValueType.UNKNOWN], 1),
            'date': MethodInfo('Date', StackValueType.DATE, [StackValueType.UNKNOWN], 1),
        }
    
    def analyze_variable_usage(self, var_idx: int, instructions: list[PCodeInstruction]) -> VariableInfo:
        ...
        for i, instr in enumerate(instructions):
            if self._instruction_references_variable(instr, var_idx):
                ...
                if instr.opcode_name.startswith('STORE') or instr.opcode_name.startswith('ASSIGN'):
                    var_info.assignments.append(instr.offset)
                else:
                    var_info.usages.append(instr.offset)
                
                # Infer type from context
                inferred_type = self._infer_type_from_instruction_context(instr, instructions, i)
    ...
    def infer_missing_operand_type(self, instruction: PCodeInstruction, operand_position: int) -> StackValueType:
        ...
        type_patterns = {
            ...
            ('EQ', 'NE', 'LT', 'GT', 'LE', 'GE'): StackValueType.UNKNOWN,  # Could be any comparable type
    ...
    def recover_variable_name(self, var_idx: int) -> str:
        ...
        if var_idx in self.variable_contexts:
            ...
            if var_info.inferred_type != StackValueType.UNKNOWN:
                return self._generate_typed_variable_name(var_idx, var_info.inferred_type)
    ...
    def _instruction_references_variable(self, instruction: PCodeInstruction, var_idx: int) -> bool:
        ...
        var_opcodes = [
            'PUSH_LOCAL_VAR', 'STORE_LOCAL_VAR', 'ASSIGN_LOCAL_VAR',
            'PUSH_GLOBAL_VAR', 'STORE_GLOBAL_VAR', 'ASSIGN_GLOBAL_VAR'
        ]
    ...
    def _infer_type_from_instruction_context(
        ...
        next_instr = all_instructions[current_index + 1] if current_index < len(all_instructions) - 1 else None
        
        # String operations context
        if opcode in ['CONCAT', 'CONCAT_STR'] or (prev_instr and prev_instr.opcode_name == 'PUSH_CONST_STRING'):
            return StackValueType.STRING
        
        # Numeric operations context
        if opcode in ['ADD', 'SUB', 'MULT', 'DIV'] or (prev_instr and prev_instr.opcode_name == 'PUSH_CONST_INT'):
            return StackValueType.INTEGER
        
        # Boolean operations context
        if opcode in ['AND', 'OR', 'NOT'] or (next_instr and next_instr.opcode_name in ['JUMPFALSE', 'JUMPTRUE']):
            return StackValueType.BOOLEAN
    ...
    def _generate_typed_variable_name(self, var_idx: int, var_type: StackValueType) -> str:
    ...
    def _infer_method_parameter_type(self, instruction: PCodeInstruction, param_position: int) -> StackValueType:
...
class ContextRecoverySystem:
    ...
    def recover_missing_operand(
        ...
        inferred_type = self.type_engine.infer_missing_operand_type(instruction, operand_position)
    ...
    def enhance_variable_names(self, instructions: list[PCodeInstruction]) -> dict[int, str]:
        ...
        for instr in instructions:
            if instr.opcode_name in ['PUSH_LOCAL_VAR', 'STORE_LOCAL_VAR'] and instr.operands:
    ...
    def analyze_control_flow_context(self, instructions: list[PCodeInstruction]) -> list[ControlFlowInfo]:
        ...
        for instr in instructions:
            if instr.opcode_name in ['JUMP', 'JUMPTRUE', 'JUMPFALSE'] and instr.operands:
