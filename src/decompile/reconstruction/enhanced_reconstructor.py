...
from .enhanced_stack import EnhancedStackManager, StackValue, StackValueType, StackValueOrigin
from .pattern_engine import PatternRecognitionEngine, PatternType
...
class EnhancedExpressionReconstructor:
    ...
    def reconstruct_block(self, block: ControlBlock) -> ReconstructionResult:
        ...
        if self.mode == ReconstructionMode.COMPREHENSIVE:
            pattern_matches = self.pattern_engine.analyze_instructions(block.instructions)
            self.reconstruction_stats['patterns_recognized'] = len(pattern_matches)
        
        # Phase 2: Context analysis  
        enhanced_locals = self.context_recovery.enhance_variable_names(block.instructions)
        self.locals.update(enhanced_locals)
        
        control_flow_info = self.context_recovery.analyze_control_flow_context(block.instructions)
        ...
        for i, instruction in enumerate(block.instructions):
            try:
                ...
                matching_patterns = [
                    ...
                    if m.start_offset == instruction.offset
                ]
    ...
    def _reconstruct_instruction(self, instruction: PCodeInstruction) -> tuple[Optional[str], float]:
        ...
        try:
            ...
            if opcode.startswith("PUSH_"):
                ...
                if not self.stack_manager.is_empty():
                    ...
                    if top_value:
                        self.stack_manager.push(StackValue(
            ...
            elif opcode.startswith("ASSIGN") or opcode.startswith("STORE"):
            ...
            elif opcode.startswith("CNV_") or opcode.startswith("CAST_"):
    ...
    def _handle_enhanced_binary_op(self, opcode: str) -> Optional[str]:
        ...
        if left.value_type == StackValueType.REAL or right.value_type == StackValueType.REAL:
            result_type = StackValueType.REAL
        elif left.value_type == StackValueType.INTEGER and right.value_type == StackValueType.INTEGER:
    ...
    def _handle_enhanced_comparison(self, opcode: str) -> Optional[str]:
    ...
    def _handle_enhanced_logical(self, opcode: str) -> Optional[str]:
        """Handle logical operations."""
        if opcode == "NOT":
            operand = self.stack_manager.pop(StackValueType.BOOLEAN)
            if not operand:
                return f"// ERROR: NOT operation failed"
            ...
            operands = self.stack_manager.pop_multiple(2, [StackValueType.BOOLEAN, StackValueType.BOOLEAN])
            if len(operands) != 2:
    ...
    def _handle_enhanced_assignment(self, opcode: str, operands: list[Any]) -> Optional[str]:
        ...
        elif not self.stack_manager.is_empty():
            lvalue = self.stack_manager.pop()
            return f"{lvalue.expression} = {value.expression}"
        
        return f"// ERROR: No target for assignment"
    
    def _handle_enhanced_call(self, opcode: str, operands: list[Any]) -> Optional[str]:
    ...
    def _handle_enhanced_return(self) -> Optional[str]:
    ...
    def _handle_enhanced_jump(self, opcode: str, operands: list[Any]) -> Optional[str]:
