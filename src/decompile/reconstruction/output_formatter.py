...
class PowerBuilderOutputFormatter:
    ...
    def __init__(self, style: OutputStyle = OutputStyle.STANDARD) -> None:
        ...
        self.pb_keywords = {
            'if', 'then', 'else', 'elseif', 'end if', 'choose', 'case', 'end choose',
            'for', 'to', 'step', 'next', 'do', 'while', 'loop', 'exit', 'continue',
            'try', 'catch', 'finally', 'end try', 'throw',
            'function', 'subroutine', 'event', 'on', 'end function', 'end subroutine', 'end event', 'end on',
            'private', 'public', 'protected', 'global', 'shared', 'readonly',
            'constant', 'ref', 'by value', 'by reference',
            'return', 'call', 'post', 'trigger', 'open', 'close',
            'create', 'destroy', 'using', 'destroy using',
        ...
        self.control_patterns = {
            ...
            'function_start': re.compile(r'^\s*(public|private|protected)?\s*(function|subroutine)\s+\w+', re.IGNORECASE),
            'function_end': re.compile(r'^\s*end\s+(function|subroutine)\s*$', re.IGNORECASE),
        }
    
    def format_statements(self, statements: list[str], confidences: Optional[list[float]] = None) -> str:
    ...
    def _calculate_indent_change(self, code: str) -> int:
        ...
        if any(pattern.match(code) for pattern in [
        ...
        # Special cases
        if self.control_patterns['else'].match(code) or self.control_patterns['elseif'].match(code):
    ...
    def _is_start_construct(self, code: str) -> bool:
        ...
        return any(pattern.match(code) for pattern in [
            self.control_patterns['if_start'],
            self.control_patterns['for_start'],
            self.control_patterns['while_start'],
            self.control_patterns['function_start'],
            ...
            if break_point in line:
                ...
                if len(parts) == 2:
                    ...
                    if len(first_part) < self.line_width - 20:  # Leave some margin
                        continuation_indent = indent_str + "    "  # Extra indent for continuation
                        return f"{first_part}{break_point}\n{continuation_indent}{second_part.strip()}"
        
        # If no good break point, just return as is
        return line
    
    def _add_documentation(self, code: str, statements: list[FormattedStatement]) -> str:
        ...
        documented_lines.append("// Confidence levels: ✓✓✓ High, ✓✓ Medium, ✓ Low, ? Very Low")
        ...
        for i, line in enumerate(lines):
            ...
            if i < len(statements):
                ...
                if stmt.confidence < 0.5 and not stmt.is_comment:
                    documented_lines.append(f"    // Note: Low confidence reconstruction")
        
        # Add footer with statistics
        total_statements = len([s for s in statements if not s.is_comment])
        high_confidence = len([s for s in statements if s.confidence >= 0.7 and not s.is_comment])
        
        documented_lines.append("")
        documented_lines.append(f"// Reconstruction summary: {high_confidence}/{total_statements} statements with high confidence")
    ...
    def _add_debug_info(self, code: str, statements: list[FormattedStatement]) -> str:
        ...
        debug_lines.append("/* Confidence Legend: ✓✓✓=High, ✓✓=Medium, ✓=Low, ?=Very Low, !!=Error */")
        debug_lines.append("")
        ...
        if statements:
            ...
            debug_lines.append(f"/* Statistics: {code_statements} code statements, {comments} comments */")
            debug_lines.append(f"/* Confidence: {confidence_dist['very_high']} very high, " +
