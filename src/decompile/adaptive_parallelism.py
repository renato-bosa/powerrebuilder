    ...
    def detect(cls) -> 'SystemProfile':
        ...
        try:
            ...
            if disk_io:
                ...
                disk_io_speed_mbps = min(100.0, max(10.0, disk_io.read_bytes / 1024 / 1024))
            else:
...
class AdaptiveParallelismEngine:
    ...
    def __init__(self):
        ...
        logger.info("System profile: CPUs=%d, Memory=%.1fGB, SSD=%s", 
    ...
    def optimize_configuration(
        ...
        if self._should_use_parallelism(file_chars):
            ...
            config.max_workers = self._calculate_optimal_workers(file_chars, config.use_processes)
            ...
            config.section_parallelism = self._should_use_section_parallelism(file_chars)
            if config.section_parallelism:
                config.add_reason("Section-level parallelism enabled")
            
            # Calculate chunk size and batch size
            config.chunk_size = self._calculate_chunk_size(file_chars, config.max_workers)
            config.batch_size = self._calculate_batch_size(file_chars, config.max_workers)
        ...
        logger.info("Parallelism configuration: workers=%d, processes=%s, confidence=%.2f",
                   config.max_workers, config.use_processes, config.confidence)
    ...
    def _calculate_optimal_workers(self, file_chars: FileCharacteristics, use_processes: bool) -> int:
    ...
    def _should_use_memory_mapping(self, file_chars: FileCharacteristics) -> bool:
        ...
        if file_chars.max_file_size_mb > 2.0 and self.system_profile.memory_available_gb > 2.0:
            return True
        
        # Use memory mapping if total workload is large
        if file_chars.total_size_mb > 20.0 and self.system_profile.memory_available_gb > 4.0:
            return True
    ...
    def _calculate_chunk_size(self, file_chars: FileCharacteristics, max_workers: int) -> int:
    ...
    def _calculate_batch_size(self, file_chars: FileCharacteristics, max_workers: int) -> int:
    ...
    def _calculate_confidence(self, file_chars: FileCharacteristics, config: ParallelismConfig) -> float:
    ...
    def record_performance(self, config: ParallelismConfig, results: Dict[str, Any]) -> None:
        ...
        logger.debug("Recorded performance data: %d records in history", 
    ...
    def get_recommended_config_summary(self, config: ParallelismConfig) -> str:
        ...
        if config.use_parallelism:
            ...
            summary_parts.append(f"Parallel processing with {config.max_workers} {parallelism_type}")
...
_adaptive_engine: Optional[AdaptiveParallelismEngine] = None
...
def optimize_for_files(
    ...
    return engine.optimize_configuration(file_paths, prefer_throughput=prefer_throughput)