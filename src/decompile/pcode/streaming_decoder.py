...
class StreamingPCodeDecoder:
    ...
    def _decode_with_memory_mapping(self, file_path: Path) -> Iterator[PCodeInstruction]:
    ...
    def _decode_with_chunked_reading(self, file_path: Path) -> Iterator[PCodeInstruction]:
        ...
        with file_path.open('rb') as f:
            ...
            while True:
                ...
                for instruction in self._extract_instructions_from_buffer(
                    ...
                    yield instruction
                    processed_bytes = instruction.offset + len(instruction.raw_bytes) - offset
    ...
    def _decode_single_instruction(
        ...
        try:
            ...
            opcode = data[offset]
            instruction_offset = absolute_offset if absolute_offset is not None else offset
    ...
    def decode_file_to_object(self, file_path: Path, object_name: str) -> DecodedObject:
        ...
        for instruction in self.decode_file_streaming(file_path):
            ...
            if (self.config.enable_instruction_batching and 
                instruction_count % self.config.batch_size == 0):
                logger.debug("Processed %d instructions from %s", 
                           instruction_count, file_path.name)
        
        logger.info("Decoded %d instructions from %s using streaming", 
                   len(instructions), file_path.name)
