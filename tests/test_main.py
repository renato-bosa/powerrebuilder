...
class TestCLI:
    ...
    @patch('main.generate_services')
    @patch('main.generate_frontend')
    def test_all_command(self, mock_frontend, mock_services, mock_models,
                        mock_decompile, mock_parse_schema, mock_parse_files,
                        mock_extract, runner):
    ...
    def test_clean_output_force(self, mock_rmtree, runner):
        ...
        with runner.isolated_filesystem():
            ...
            result = runner.invoke(cli, ['clean_output', 'output/extracted/recovery', '--force'])
    ...
    def test_cli_traceback_option(self, runner):
        ...
        with patch('main.extract_pbls', side_effect=Exception("Test error")):
            with runner.isolated_filesystem():
                ...
                result = runner.invoke(cli, ['--traceback', 'extract', str(input_dir), 'output'])
                assert result.exit_code == 1
