...
class PowerBuilderTestData:
    ...
    def create_pbd_file(path: Path, objects: List[Dict[str, Any]]) -> None:
        ...
        with open(path, 'wb') as f:
            ...
            for obj in objects:
                ...
                current_data_offset += len(obj.get('pcode', b'')) + 8  # Include DAT header
    ...
    def create_sru_window(name: str, controls: List[Dict[str, Any]]) -> str:
        ...
        lines.append(f'    this.Control[]={{' + ','.join(f'this.{c["name"]}' for c in controls) + '}')
        lines.append('end on')
        lines.append('')
    ...
    @staticmethod
    def create_sru_datawindow(name: str, sql: str, columns: List[Dict[str, Any]]) -> str:
        ...
        x_pos = 10
        for col in columns:
            lines.append(f'column(band=detail id={columns.index(col)+1} alignment="0" tabsequence={columns.index(col)+1}0 border="0" color="33554432" x="{x_pos}" y="4" height="76" width="{col.get("width", 274)}" format="[general]" html.valueishtml="0"  name={col["name"]} visible="1" edit.limit=0 edit.case=any edit.autoselect=yes edit.autohscroll=yes  font.face="Arial" font.height="-10" font.weight="400" font.pitch="2" font.charset="0" background.mode="1" background.color="536870912" )')
            x_pos += col.get("width", 274) + 10
        
        return '\n'.join(lines)
    
    @staticmethod
    def create_sru_nonvisual(name: str, variables: List[Dict], methods: List[Dict]) -> str:
        ...
        if methods:
            ...
            for method in methods:
                ...
                if method.get('is_function', True):
                    lines.append(f'{visibility} function {return_type} {method["name"]} ({params})')
                else:
...
class TestDataGenerator:
    ...
    def create_simple_application(root_dir: Path) -> None:
        ...
        app_sru = PowerBuilderTestData.create_sru_nonvisual(
            "test_app",
            variables=[
                {"type": "string", "name": "is_app_name", "initial_value": "Test Application"}
            ],
            methods=[
                {
                    "name": "open",
        ...
        dw_sru = PowerBuilderTestData.create_sru_datawindow(
            ...
            columns=[
                ...
                {"name": "salary", "type": "decimal(10,2)", "dbname": "employee.salary"}
            ]
        )
    ...
    def create_complex_inheritance_test(root_dir: Path) -> None:
        ...
        base_sru = PowerBuilderTestData.create_sru_nonvisual(
            "n_base",
            variables=[
                {"type": "string", "name": "is_type", "visibility": "protected", "initial_value": "base"}
            ],
            methods=[
                {
                    "name": "of_get_type",
