...
def test_return_validation(validator, type_registry):
    ...
    assert ReturnStatement().validate({'type_registry': type_registry, 'expected_type': None})

    # Return with value in non-void context
    int_type = type_registry.get_type("INTEGER")
    return_stmt = ReturnStatement(Expression())
    assert return_stmt.validate({'type_registry': type_registry, 'expected_type': int_type})

    # Return with value in void context should fail
    assert not return_stmt.validate({'type_registry': type_registry, 'expected_type': None})
