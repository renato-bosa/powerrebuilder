...
class OpcodeReferenceGenerator:
    ...
    def load_reference(self):
        ...
        if json_path.exists():
            ...
            raise FileNotFoundError("Run extract_all_opcodes.py first to generate reference")
    
    def generate_python_implementation(self):
        """Generate Python opcode implementation."""
        print("🐍 Generating Python implementation...")
        ...
        with open(output_path, 'w') as f:
            ...
            for opcode_hex, info in self.reference['opcodes'].items():
                ...
                f.write(f'    0x{opcode:02X}: OpcodeInfo({opcode}, "{name}", {length}, ')
            ...
            if 'patterns' in self.reference and 'type_variants' in self.reference['patterns']:
                f.write('# Type-specific opcode variants\n')
                f.write('TYPE_VARIANTS = {\n')
                
                for base_op, types in self.reference['patterns']['type_variants'].items():
        ...
        print(f"✅ Generated Python implementation at {output_path}")
    
    def generate_csharp_implementation(self):
        ...
        with open(output_path, 'w') as f:
            ...
            f.write('        private static readonly Dictionary<ushort, OpcodeInfo> _opcodes = new()\n')
            f.write('        {\n')
        ...
        print(f"✅ Generated C# implementation at {output_path}")
    
    def generate_test_framework(self):
        """Generate test framework for opcode verification."""
        print("🧪 Generating test framework...")
        ...
        with open(output_path, 'w') as f:
            ...
            f.write('from decompile.opcodes_unified import OPCODES, get_opcode_name, get_opcode_length\n\n')
            ...
            f.write('        """Verify type-specific variants exist for common operations."""\n')
            f.write('        # Operations that should have type variants\n')
            f.write('        expected_variants = ["ADD", "SUB", "MUL", "DIV", "ASSIGN", "PUSH"]\n')
            f.write('        \n')
            f.write('        for base_op in expected_variants:\n')
            f.write('            variants = [name for name in [info.name for info in OPCODES.values()] \n')
            f.write('                       if name.startswith(base_op + "_")]\n')
            f.write('            assert len(variants) > 1, f"{base_op} should have type variants"\n\n')
            ...
            for opcode, name in known_opcodes:
        ...
        print(f"✅ Generated test framework at {output_path}")
    
    def generate_comparison_report(self):
        """Generate a report comparing implementations."""
        print("📊 Generating implementation comparison report...")
        ...
        with open(output_path, 'w') as f:
            ...
            f.write("## Overview\n\n")
            f.write("This document compares opcode implementations across different decompilers.\n\n")
            ...
            f.write(f"- Opcodes only in powerbuilder-decompile: {source_counts['powerbuilder-decompile']}\n")
            ...
            for opcode_hex, info in self.reference['opcodes'].items():
                ...
                if len(impls) > 1:
                    ...
                    py_func = impls.get('powerbuilder-decompile', {}).get('function', '-')
            ...
            for diff in differences[:20]:  # Show first 20
                f.write(f"| {diff['opcode']} | {diff['pb']} | {diff['py']} | {diff['name']} |\n")
...
def main():
    ...
    print("\n✨ Opcode reference generation complete!")
    print("\nGenerated files:")
    print("  - decompile/opcodes_unified.py (Python implementation)")
    print("  - reference/implementations/Opcodes.cs (C# implementation)")
    print("  - tests/opcode_verification/test_opcodes.py (Test framework)")
    print("  - docs/implementation_comparison.md (Comparison report)")
    print("\nNext steps:")
    print("  1. Run tests: pytest tests/opcode_verification/")
    print("  2. Update decoder to use opcodes_unified.py")
    print("  3. Compare decompilation results with reference implementations")
