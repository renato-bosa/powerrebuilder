...
from ..data.database_operation_formatter import DatabaseOperationFormatter
...
class MethodBodyConverter:
    ...
    def _convert_variable_declaration(self, match: re.Match, context: dict[str, Any] = None) -> ConvertedStatement:
        ...
        if initial_value:
            dart_value = self.expression_converter.convert_expression(initial_value.strip())
    ...
    def _convert_assignment(self, match: re.Match, context: dict[str, Any] = None) -> ConvertedStatement:
        ...
        if "." in rhs and context and "controls" in context:
            ...
            if control_name in context.get("controls", {}):
                ...
                if flutter_widget.get("requires_controller") and property_name.lower() == "text":
        ...
        if "." in lhs and context and "controls" in context:
            ...
            if control_name in context.get("controls", {}):
                return self._convert_control_property_assignment(control_name, property_name, rhs, context)
    ...
    def _convert_database_operation(self, match: re.Match, context: dict[str, Any] = None) -> ConvertedStatement:
        ...
        if db_ops and db_ops[0]:
            ...
            dart_code = db_ops[0] if self.db_formatter.target == "flutter" else self._generate_dart_db_op(full_statement)
            python_code = db_ops[0] if self.db_formatter.target == "python" else self._generate_python_db_op(full_statement)

            return ConvertedStatement(
                dart_code=dart_code,
                python_code=python_code,
                requires_async=True,
                imports_needed=["dart:async"] if self.db_formatter.target == "flutter" else ["sqlalchemy"],
            )
    ...
    def _convert_ui_method(self, obj: str, method: str, args: str, 
        ...
        if method_lower == "setfocus":
            ...
            python_code = f'self.{obj}.{method}({self._convert_arguments(args, "python")})'
    ...
    def _convert_arguments(self, args: str, target: str) -> str:
        ...
        if target == "dart":
            converted = [self.expression_converter.convert_expression(arg.strip()) for arg in arg_list]
        else:
            converted = [self._convert_expression_to_python(arg.strip()) for arg in arg_list]
    ...
    def _get_indent_change(self, line: str) -> int:
        ...
        # Opening statements
        if any(line_lower.startswith(kw) for kw in ["if ", "for ", "do ", "while ", "try", "choose case"]):
            if not line_lower.endswith("then"):
                return 0
            return 1

        # Closing statements
        if any(line_lower.startswith(kw) for kw in ["end if", "next", "loop", "end try", "end choose"]):
            return -1

        # Middle statements
        if any(line_lower.startswith(kw) for kw in ["else", "elseif", "catch", "finally", "case"]):
            return 0
    ...
    def _is_async_method(self, method_name: str, context: dict[str, Any] = None) -> bool:
        ...
        async_patterns = [
            "fetch", "load", "save", "get", "post", "put", "delete",
            "query", "execute", "commit", "rollback", "connect",
        ]
...
final results = await database.query(
  ...
  """
        ...
        elif statement_lower == "commit":
            return "// Commit transaction\nawait database.commit();"
        elif statement_lower == "rollback":
