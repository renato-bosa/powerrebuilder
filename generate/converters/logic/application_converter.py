...
class ApplicationConverter:
    ...
    def _extract_properties(self, syntax: str, definition: ApplicationDefinition) -> None:
        ...
        # Micro help settings
        micro_help_match = re.search(r"microhelp\s*=\s*(true|false)", syntax, re.IGNORECASE)
        if micro_help_match:
            definition.micro_help = micro_help_match.group(1).lower() == "true"

        dynamic_help_match = re.search(r"dynamicmicrohelp\s*=\s*(true|false)", syntax, re.IGNORECASE)
        if dynamic_help_match:
            definition.dynamic_micro_help = dynamic_help_match.group(1).lower() == "true"

        # Toolbar settings
        toolbar_text_match = re.search(r"toolbartext\s*=\s*(true|false)", syntax, re.IGNORECASE)
        if toolbar_text_match:
            definition.toolbar_text = toolbar_text_match.group(1).lower() == "true"

        toolbar_tips_match = re.search(r"toolbartips\s*=\s*(true|false)", syntax, re.IGNORECASE)
    ...
    def _extract_variables(self, syntax: str) -> list[ApplicationVariable]:
        ...
        instance_pattern = r"(?:public|private|protected)?\s*(\w+)\s+(\w+)(?:\s*=\s*([^;\n]+))?"
        ...
        for pb_type, var_name, initial_value in instance_matches:
            ...
            if pb_type.lower() in ["global", "type", "from", "end", "on", "event"]:
                continue
    ...
    def _extract_events(self, syntax: str) -> list[ApplicationEvent]:
        ...
        event_names = ["open", "close", "idle", "systemerror", "connectionbegin", "connectionend"]
                ...
                if params_str:
                    ...
                    for param in param_parts:
                        ...
                        if " " in param:
                            ...
                            event.parameters.append((param_type.strip(), param_name.strip()))
    ...
    def _extract_database_config(self, syntax: str, definition: ApplicationDefinition) -> None:
        ...
        for prop, value in sqlca_matches:
            ...
            if prop_lower == "dbms":
                definition.dbms = value
            elif prop_lower == "database":
                definition.database = value
            elif prop_lower == "userid":
                definition.userid = value
            elif prop_lower == "dbpass":
                definition.db_pass = value
            elif prop_lower == "logid":
                definition.log_id = value
            elif prop_lower == "logpass":
                definition.log_pass = value
            elif prop_lower == "servername":
                definition.server_name = value
    ...
    def generate_flutter_main(self, app_def: ApplicationDefinition) -> dict[str, list[str]]:
        ...
        if app_def.initial_window:
            ...
            code["imports"].append(f"import 'windows/{self._to_snake_case(window_name)}.dart';")
        ...
        if app_def.variables:
            ...
            for var in app_def.variables:
                if var.is_global:
                    ...
                    code["globals"].append(f"late {var.dart_type} {var.name} = {init_val};")
    ...
    def _generate_flutter_app_class(self, app_def: ApplicationDefinition) -> list[str]:
        ...
        if app_def.initial_window:
            ...
            lines.append('        appBar: AppBar(title: Text("' + app_def.display_name + '")),')
            lines.append('        body: Center(child: Text("No initial window specified")),')
            lines.append("      ),")
    ...
    def _generate_flutter_events(self, app_def: ApplicationDefinition) -> list[str]:
        ...
        for event in app_def.events:
            ...
            if event.name == "open":
                ...
                params = ", ".join(f"{self.type_converter.convert_type(t)} {n}" 
    ...
    def generate_python_main(self, app_def: ApplicationDefinition) -> dict[str, list[str]]:
        ...
        code["imports"] = [
            "#!/usr/bin/env python3",
            '"""' + app_def.display_name + ' - Generated from PowerBuilder Application"""',
            "",
            "import sys",
        ...
        if app_def.initial_window:
            window_name = self._to_pascal_case(app_def.initial_window)
            code["imports"].append(f"from .windows.{self._to_snake_case(window_name)} import {window_name}")
        ...
        if app_def.variables:
            ...
            for var in app_def.variables:
                if var.is_global:
                    init_val = var.initial_value or self._get_python_default_value(var.python_type)
                    code["globals"].append(f"{var.name}: {var.python_type} = {init_val}")
    ...
    def _generate_python_app_class(self, app_def: ApplicationDefinition) -> list[str]:
        ...
        lines.append('        self.root.protocol("WM_DELETE_WINDOW", self._application_close)')
        lines.append("        ")
        ...
        if app_def.initial_window:
            ...
            lines.append('        label = ttk.Label(self.root, text="No initial window specified")')
    ...
    def _generate_python_events(self, app_def: ApplicationDefinition) -> list[str]:
        ...
        for event in app_def.events:
            lines.append("    ")
            params = ["self"] + [f"{n}: {self._convert_to_python_type(t)}" 
                               for t, n in event.parameters]
    ...
    def _generate_python_database_init(self, app_def: ApplicationDefinition) -> list[str]:
        ...
        if app_def.dbms:
            dbms_lower = app_def.dbms.lower()

            if "sqlite" in dbms_lower:
                db_path = app_def.database or "app.db"
                lines.append(f'            self.engine = create_engine("sqlite:///{db_path}")')
            elif "postgresql" in dbms_lower or "postgres" in dbms_lower:
                lines.append("            connection_string = (")
                lines.append(f'                "postgresql://{app_def.userid or "user"}:"')
                lines.append(f'                "{app_def.db_pass or "password"}@"')
                lines.append(f'                "{app_def.server_name or "localhost"}/"')
                lines.append(f'                "{app_def.database or "database"}"')
                lines.append("            )")
                lines.append("            self.engine = create_engine(connection_string)")
            elif "mysql" in dbms_lower:
                lines.append("            connection_string = (")
                lines.append(f'                "mysql+pymysql://{app_def.userid or "user"}:"')
                lines.append(f'                "{app_def.db_pass or "password"}@"')
                lines.append(f'                "{app_def.server_name or "localhost"}/"')
                lines.append(f'                "{app_def.database or "database"}"')
                lines.append("            )")
                ...
                lines.append(f"            # TODO: Configure {app_def.dbms} connection")
                lines.append("            self.engine = None")
        else:
        ...
        lines.append("                self.session_factory = sessionmaker(bind=self.engine)")
        lines.append("                ")
        lines.append('                logging.info("Database initialized successfully")')
        lines.append("        except Exception as e:")
        lines.append('            logging.error(f"Failed to initialize database: {e}")')
        lines.append('            messagebox.showerror("Database Error", f"Failed to connect to database: {e}")')
