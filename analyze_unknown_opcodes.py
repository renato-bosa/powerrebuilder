...
def analyze_unknown_opcodes():
    ...
    print("=== Top 30 Single Opcodes ===")
    for opcode, count in single_counts.most_common(30):
        print(f'{count:5d} {opcode}')
        
    print("\n=== Top 30 Opcode-Variant Pairs ===")
    ...
    for pair, count in sorted(e4_variants.items(), key=lambda x: x[1], reverse=True):
        print(f'{count:5d} {pair}')
        
    print("\n=== E0 Variants ===")
    e0_variants = {k: v for k, v in pair_counts.items() if k.startswith('0xE0')}
    for pair, count in sorted(e0_variants.items(), key=lambda x: x[1], reverse=True):
        print(f'{count:5d} {pair}')
        
    print("\n=== E1 Variants ===")
    e1_variants = {k: v for k, v in pair_counts.items() if k.startswith('0xE1')}
    for pair, count in sorted(e1_variants.items(), key=lambda x: x[1], reverse=True):
        print(f'{count:5d} {pair}')
