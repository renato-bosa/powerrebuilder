...
def find_never_imported_files(analysis_data: Dict[str, Any], src_dir: Path) -> List[str]:
        ...
        if module_parts:
            ...
            for imp in all_imports:
                if (module_name in imp or 
                    imp in module_name or
                    module_parts[-1] in imp if module_parts else False):
                    is_imported = True
                    break
...
def find_unused_functions_and_classes(analysis_data: Dict[str, Any]) -> Dict[str, List[str]]:
    ...
    special_names = {
        ...
        'setUp', 'tearDown', 'test_', 'main', 'cli'
    }
    
    for func in all_functions:
        if (func not in all_usage and 
            not func.startswith('test_') and
            not any(special in func for special in special_names)):
    ...
    for method in all_methods:
        ...
        if (method not in all_usage and 
            method_name not in all_usage and
            not method_name.startswith('test_') and
            not any(special in method_name for special in special_names)):
...
def find_large_files(src_dir: Path, size_threshold: int = 1000) -> List[Tuple[str, int]]:
...
def analyze_test_coverage(analysis_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    non_test_files = [f for f in analysis_data['python_files'] if 'test' not in f.lower()]
    ...
    return {
        ...
        'test_coverage_ratio': len(test_files) / len(non_test_files) if non_test_files else 0,
...
def main():
    ...
    print(f"========================")
    print(f"Total Python files analyzed: {results['analysis_summary']['total_python_files']}")
