...
class SyntaxErrorFixer:
    ...
    def fix_unmatched_else(self, lines: List[str], error_line: int) -> List[str]:
        ...
        if error_line > 0 and error_line <= len(lines):
            ...
            indent_level = len(lines[error_line - 1]) - len(lines[error_line - 1].lstrip())
            ...
            for i in range(error_line - 2, -1, -1):
                ...
                if line.strip() and not line.strip().startswith('#'):
                    ...
                    if line_indent < indent_level and any(line.strip().startswith(kw) for kw in ['if ', 'elif ', 'try:', 'except']):
                        # Found the matching block, fix else indentation
                        lines[error_line - 1] = ' ' * line_indent + lines[error_line - 1].strip()
                        return lines
        return lines
    ...
    def fix_unmatched_parenthesis(self, lines: List[str], error_line: int, error_type: str) -> List[str]:
    ...
    def fix_invalid_syntax(self, lines: List[str], error_line: int, error_info: Dict) -> List[str]:
        """Fix various invalid syntax errors."""
        if error_line > 0 and error_line <= len(lines):
            line = lines[error_line - 1]
            
            # Check for missing colons
            if any(keyword in line for keyword in ['if ', 'elif ', 'else', 'try', 'except', 'finally', 'def ', 'class ', 'for ', 'while ', 'with ']):
            ...
            if line.strip().startswith(('elif ', 'else:')):
                ...
                for i in range(error_line - 2, -1, -1):
                    prev = lines[i]
                    if prev.strip() and (prev.strip().startswith('if ') or prev.strip().startswith('elif ')):
    ...
    def fix_file(self, filepath: Path) -> bool:
        ...
        print(f"\nFixing {filepath}")
        print(f"  Error: {error_info['error']} at line {error_info['line']}")
        print(f"  Text: {error_info['text'][:80]}...")
        ...
        if "unexpected indent" in error_info['error']:
            ...
            lines = self.fix_unmatched_parenthesis(lines, error_info['line'], error_info['error'])
        ...
        if lines != original_lines:
            try:
                ...
                if self.detect_syntax_error(filepath) is None:
                    ...
                    print("  ✗ Fix didn't resolve the issue, reverting")
                    with open(filepath, 'w', encoding='utf-8') as f:
            ...
            print("  ℹ No automated fix available")
    ...
    def fix_all(self):
        ...
        for pass_num in range(max_passes):
            print(f"\n{'='*60}")
            print(f"Pass {pass_num + 1} of {max_passes}")
            print(f"{'='*60}")
        ...
        print(f"Manual fix needed: {len(self.files_with_errors)} files")
        
        if self.files_with_errors:
            print("\nFiles requiring manual intervention:")
            for f in sorted(self.files_with_errors):
                print(f"  - {f}")
